<x-unified-layout title="Customer Dashboard" subtitle="Your Sports Ticket Discovery Hub">
  <x-slot name="headerActions">
    <div class="flex items-center space-x-3">
      <!-- Subscription Status Indicator -->
      @if(Auth::user()->hasActiveSubscription())
        <x-ui.badge variant="success" dot="true">Premium Active</x-ui.badge>
      @else
        <x-ui.badge variant="warning" dot="true">Free Trial</x-ui.badge>
      @endif
        ...
        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border z-50">
          <div class="p-4">
            ...
            <div class="mt-3 pt-3 border-t">
              <a href="{{ route('tickets.alerts.index') }}" class="text-sm text-blue-600 hover:text-blue-700">View all alerts →</a>
            </div>
          </div>
        </div>
      </div>
                ...
                <div class="bg-white/20 rounded-lg p-3 mb-3">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                      ...
                      <p class="text-white font-semibold text-sm">{{ Auth::user()->subscription->next_billing_date?->format('M j, Y') ?? 'N/A' }}</p>
                    </div>
                  </div>
                </div>
              @else
                <div class="bg-yellow-500/20 rounded-lg p-3 mb-3">
                  <div class="flex items-center justify-between">
                    <div>
                      ...
                      <p class="text-white/80 text-sm">{{ Auth::user()->getFreeTrialDaysRemaining() }} days remaining</p>
                    </div>
                    <a href="{{ route('subscription.plans') }}" class="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg text-white text-sm font-medium transition">
                      Upgrade Now
                    </a>
                  </div>
                </div>
            ...
            <div class="text-right">
              <div class="bg-white/20 rounded-lg p-4 min-w-[200px]">
                ...
                <div class="w-full bg-white/20 rounded-full h-2">
                  <div class="bg-white rounded-full h-2 transition-all duration-300" style="width: {{ min($percentage, 100) }}%"></div>
                </div>
        ...
        <x-ui.card-content>
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="hd-heading-2 !text-white !mb-0" x-text="stats.active_alerts || '{{ Auth::user()->activeAlerts()->count() }}'">
                {{ Auth::user()->activeAlerts()->count() }}
              </p>
              <div class="mt-1">
                @if(Auth::user()->recentAlerts()->count() > 0)
                  <x-ui.badge variant="warning" size="xs" dot="true">{{ Auth::user()->recentAlerts()->count() }} new</x-ui.badge>
                @else
            ...
            <div>
              ...
              <div class="mt-1">
                @if(Auth::user()->ticketPurchases()->recent()->count() > 0)
                  <x-ui.badge variant="info" size="xs" dot="true">{{ Auth::user()->ticketPurchases()->recent()->count() }} recent</x-ui.badge>
                @else
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <x-ui.card>
          <x-ui.card-header title="Quick Actions">
            <x-ui.button href="{{ route('tickets.scraping.index') }}" variant="outline" size="sm">Browse All</x-ui.button>
          </x-ui.card-header>
          <x-ui.card-content>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <a href="{{ route('tickets.scraping.index', ['sport' => 'football']) }}" class="flex flex-col items-center p-4 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors group">
                <div class="w-12 h-12 bg-orange-500 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
              ...
              <a href="{{ route('tickets.scraping.index', ['sport' => 'basketball']) }}" class="flex flex-col items-center p-4 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors group">
                <div class="w-12 h-12 bg-orange-500 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <span class="text-white font-bold text-lg">🏀</span>
                </div>
              ...
              <a href="{{ route('tickets.scraping.index', ['sport' => 'baseball']) }}" class="flex flex-col items-center p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors group">
              ...
              <a href="{{ route('tickets.scraping.index', ['sport' => 'soccer']) }}" class="flex flex-col items-center p-4 bg-green-50 rounded-lg hover:bg-green-100 transition-colors group">
        ...
        <x-ui.card>
          <x-ui.card-header title="Trending Events">
            <x-ui.button href="{{ route('tickets.scraping.trending') }}" variant="ghost" size="sm">View All</x-ui.button>
          </x-ui.card-header>
          <x-ui.card-content>
            <div class="space-y-4">
              @forelse($trendingEvents ?? [] as $event)
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <div class="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 class="font-semibold text-gray-900">{{ $event['title'] ?? 'Kansas City Chiefs vs Patriots' }}</h4>
                      <p class="text-sm text-gray-500">{{ $event['venue'] ?? 'Arrowhead Stadium' }} • {{ $event['date'] ?? 'Dec 15, 2024' }}</p>
                    </div>
                  </div>
                  <div class="text-right">
      ...
      <div class="space-y-6">
        ...
        <x-ui.card>
          <x-ui.card-header title="Recent Alerts">
            <x-ui.button href="{{ route('tickets.alerts.index') }}" variant="ghost" size="sm">Manage</x-ui.button>
          </x-ui.card-header>
          <x-ui.card-content>
            <div class="space-y-3">
              ...
              @empty
                <div class="text-center py-6 text-gray-500">
                  ...
                  <a href="{{ route('tickets.alerts.create') }}" class="text-blue-600 hover:text-blue-700 text-sm">Create your first alert →</a>
                </div>
              @endforelse
            </div>
          </x-ui.card-content>
          ...
          <x-ui.card-content>
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                @endphp
                <span class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">{{ $completion['percentage'] }}%</span>
              </div>
              ...
              <div class="pt-3 border-t">
                <div class="space-y-2">
                  @if(!Auth::user()->hasActiveSubscription())
                    <a href="{{ route('subscription.plans') }}" class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition text-center block">
                      Upgrade to Premium
                    </a>
                  @endif
                  <a href="{{ route('profile.edit') }}" class="w-full bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition text-center block">
                    Manage Account
                  </a>
                </div>
              </div>
