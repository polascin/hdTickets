            ...
            <div class="card mb-4">
                <div class="card-body text-center">
                    <div class="position-relative d-inline-block mb-3">
                        ...
                        @endif
                        <span class="position-absolute bottom-0 end-0 bg-{{ $user->is_active ? 'success' : 'danger' }} 
                                     rounded-circle" style="width: 24px; height: 24px; border: 3px solid white;"></span>
                    </div>
                    ...
                    <div class="d-flex justify-content-center mb-3">
                        <span class="badge bg-{{ $user->role === 'admin' ? 'danger' : ($user->role === 'agent' ? 'primary' : ($user->role === 'scraper' ? 'secondary' : 'success')) }} me-2">
                            <i class="fas fa-{{ $user->role === 'admin' ? 'shield-alt' : ($user->role === 'agent' ? 'user-tie' : ($user->role === 'scraper' ? 'robot' : 'user')) }}"></i>
                            {{ ucfirst($user->role) }}
                        </span>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="progress ms-2 flex-grow-1" style="height: 8px;">
                                        <div class="progress-bar" style="width: {{ min(($user->activity_score ?? 0) / 100 * 100, 100) }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <div>
                            <span class="badge bg-{{ $user->role === 'admin' ? 'danger' : ($user->role === 'agent' ? 'primary' : ($user->role === 'scraper' ? 'secondary' : 'success')) }} fs-6 me-2">
                                <i class="fas fa-{{ $user->role === 'admin' ? 'shield-alt' : ($user->role === 'agent' ? 'user-tie' : ($user->role === 'scraper' ? 'robot' : 'user')) }}"></i>
                                {{ ucfirst($user->role) }}
                            </span>
                        ...
                        <div class="row">
                            ...
                            @foreach($keyPermissions as $permission => $label)
                                <div class="col-md-6 mb-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-{{ $permissions[$permission] ? 'check text-success' : 'times text-danger' }} me-2"></i>
                    ...
                    @if(auth()->user()->canManageUsers())
                        <div class="d-flex gap-2">
                            <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#changeRoleModal">
                                <i class="fas fa-exchange-alt"></i> Change Role
                            </button>
                            <button class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#customPermissionsModal">
                    ...
                    <div class="timeline">
                        @forelse($activities ?? [] as $activity)
                            <div class="timeline-item mb-3">
                                <div class="d-flex">
                                    <div class="timeline-marker bg-primary rounded-circle me-3" style="width: 10px; height: 10px; margin-top: 6px;"></div>
