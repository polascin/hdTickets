<x-modern-app-layout title="Sports Tickets" subtitle="Browse and search for sports tickets across multiple platforms">
    <x-slot name="headerActions">
        <div class="flex gap-3">
            <x-ui.button id="refresh-tickets" icon="refresh">
                Refresh
            </x-ui.button>
    ...
    <x-ui.card class="mb-6">
        ...
        <x-ui.card-content>
            <form id="filters-form" class="space-y-6">
                <!-- Search Input -->
                <x-ui.input name="keywords" label="Search Tickets" placeholder="Search by event, team, venue..."
                    value="{{ request('keywords') }}" variant="search" icon="search" iconPosition="right" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <!-- Min Price -->
                    <x-ui.input name="min_price" type="number" label="Min Price" placeholder="0"
                        value="{{ request('min_price') }}" />

                    <!-- Max Price -->
                    <x-ui.input name="max_price" type="number" label="Max Price" placeholder="1000"
                ...
                <div class="flex flex-wrap gap-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="high_demand" value="1"
            ...
            <x-ui.card hover="true" class="ticket-card">
                <x-ui.card-content>
                    ...
                    @if ($ticket->is_available)
                        <x-ui.button href="{{ $ticket->ticket_url }}" target="_blank" fullWidth="true"
                            icon="external-link" iconPosition="right">
                            View Tickets
                        </x-ui.button>
                    @endif
                </x-ui.card-content>
    ...
    <div id="empty-state" class="text-center py-12"
        style="display: {{ $tickets->count() > 0 ? 'none' : 'block' }};">
    ...
    @if ($tickets->hasPages())
        <div class="text-center mt-8">
            <x-ui.button id="load-more-btn" variant="outline" size="lg">
                <span class="btn-text">Load More Tickets</span>
                <svg class="animate-spin -mr-1 ml-3 h-5 w-5 text-gray-700" xmlns="http://www.w3.org/2000/svg"
                ...
                <x-ui.card-content>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="hd-heading-2 !text-white !mb-0" id="stat-total">
                ...
                <x-ui.card-content>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="hd-heading-2 !text-white !mb-0" id="stat-available">
                                {{ $tickets->where('is_available', true)->count() }}</p>
                        </div>
                ...
                <x-ui.card-content>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="hd-heading-2 !text-white !mb-0" id="stat-high-demand">
                                {{ $stats['high_demand_tickets'] ?? 0 }}</p>
                        </div>
                ...
                <x-ui.card-content>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="hd-heading-2 !text-white !mb-0" id="stat-platforms">
                                {{ $stats['active_alerts'] ?? 0 }}</p>
                        </div>
                        <div class="bg-purple-400 bg-opacity-50 rounded-full p-3">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                                <path
                                    d="M10 2L3 7v11c0 5.55 3.84 10 9 10s9-4.45 9-10V7l-7-5zM8 13l2-2 1-1 4-4-1.5-1.5L9 9 8 8l-1.5 1.5L8 13z">
                                </path>
                            </svg>
                        </div>
                    </div>
