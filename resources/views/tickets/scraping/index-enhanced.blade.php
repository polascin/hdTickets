<x-unified-layout title="Sports Tickets" subtitle="Browse and search for sports tickets across multiple platforms">
    ...
    @if (isset($activeFilters) && array_filter($activeFilters))
        <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg" role="region" aria-label="Active filters">
            <div class="flex items-start justify-between">
                            ...
                            @if ($value && $value !== '' && $value !== false)
                                <span
                                    ...
                                    @else
                                        {{ $value }}
                                    @endif
                                    <button type="button" class="ml-1 text-blue-400 hover:text-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded"
                ...
                <button data-clear-filters class="text-blue-600 hover:text-blue-800 text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded px-2 py-1">
                    Clear All
                </button>
            </div>
        </div>
            ...
            <form id="filters-form" class="space-y-6" role="search" aria-label="Ticket search and filtering">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        <select name="platform" id="platform" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <select name="sort_by" id="sort_by" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            <option value="scraped_at" {{ request('sort_by', 'scraped_at') == 'scraped_at' ? 'selected' : '' }}>Latest</option>
                ...
                <fieldset class="space-y-4">
                    ...
                    <div class="flex flex-wrap gap-6">
                        ...
                        <button type="button" id="advanced-filters-toggle"
                            ...
                            Advanced Filters
                            <svg class="w-3 h-3 ml-1 transition-transform" id="advanced-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <div id="advanced-filters" class="hidden border-t pt-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="date_from" id="date_from" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <input type="date" name="date_to" id="date_to" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <input type="text" name="venue" id="venue" placeholder="Enter venue name" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                value="{{ request('venue') }}" />
                        </div>
                    </div>
                </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div class="flex items-center gap-2">
            <button id="grid-view-toggle" class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors focus:ring-2 focus:ring-gray-500" 
            ...
            <button id="list-view-toggle" class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors focus:ring-2 focus:ring-gray-500" 
    ...
    <!-- Loading Indicator -->
    <div id="loading-indicator" class="flex flex-col items-center justify-center py-12 hidden" role="status" aria-label="Loading tickets">
    ...
    <div id="tickets-container">
        ...
        <div id="grid-tickets" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            @forelse ($tickets ?? collect() as $ticket)
                <article class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-blue-300 transition-all duration-200 group focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2">
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            @if (($ticket->is_high_demand ?? false))
                                <span class="ml-2 flex-shrink-0 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 animate-pulse"
                            ...
                            @if (($ticket->quantity_available ?? null))
                                <div class="mt-3 flex items-center justify-between">
                                    <p class="text-xs text-gray-600 flex items-center">
                                        <svg class="w-3 h-3 mr-1 text-green-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                            <path fill-rule="evenodd"
        ...
        <div id="list-tickets" class="space-y-4 mb-8 hidden">
            @forelse ($tickets ?? collect() as $ticket)
                <article class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-blue-300 transition-all duration-200 focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2">
                    <div class="p-6">
    ...
    @if (($tickets ?? collect())->hasPages())
        <nav class="flex flex-col sm:flex-row justify-between items-center bg-white border border-gray-200 rounded-lg px-6 py-4 shadow-sm mb-8"
    ...
    <script>
        ...
        function viewTicketDetails(ticketId) {
            ...
            modal.className = 'hd-modal-backdrop fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4';
        ...
        function displayTicketDetails(ticket, container, modal) {
            ...
            const getStatusBadge = (status, isAvailable) => {
                if (!isAvailable) return '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800">Sold Out</span>';
                if (status === 'active') return '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800">Available</span>';
                return '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800">' + (status || 'Unknown').charAt(0).toUpperCase() + (status || 'unknown').slice(1) + '</span>';
            };
            ...
            const getRecommendationBadge = (score) => {
                if (score >= 80) return '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800">🌟 Highly Recommended</span>';
                if (score >= 60) return '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">👍 Recommended</span>';
        ...
        function createAlert(ticketId = null) {
            ...
            modal.querySelector('#create-alert-form').onsubmit = function(e) {
                ...
                announceToScreenReader('Sports ticket alert created successfully! You will be notified when matching tickets are found.');
                document.body.removeChild(modal);
            };
