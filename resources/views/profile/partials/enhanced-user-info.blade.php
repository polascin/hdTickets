<section>
    <header class="mb-6">
        <h2 class="text-lg font-medium text-gray-900">
            {{ __('Enhanced User Information') }}
        </h2>
        <p class="mt-1 text-sm text-gray-600">
            {{ __('Comprehensive user profile information, activity stats, and system permissions.') }}
        </p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div class="flex items-center space-x-4 mb-4">
                <div class="flex-shrink-0">
                    @if($profile['has_picture'])
                        <img class="w-16 h-16 rounded-full object-cover" src="{{ $profile['picture_url'] }}" alt="{{ $profile['display_name'] }}">
                    @else
                        <div class="w-16 h-16 bg-gray-300 rounded-full flex items-center justify-center">
                            <span class="text-lg font-medium text-gray-700">{{ $profile['initials'] }}</span>
                        </div>
                    @endif
                </div>
                <div>
                    <h4 class="text-lg font-semibold text-gray-900">{{ $profile['full_name'] ?: 'No name set' }}</h4>
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <span class="text-gray-600">{{ $user->phone ?: 'Not provided' }}</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Timezone</label>
                    <span class="text-gray-600">{{ $profile['timezone'] }}</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Language</label>
                    <span class="text-gray-600">{{ strtoupper($profile['language']) }}</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">User ID</label>
                    <span class="text-gray-600 font-mono text-xs">{{ $user->id }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            <h3 class="text-md font-semibold text-gray-900 mb-4 flex items-center">
                <svg class="w-5 h-5 mr-2 text-{{ $permissions['role_display']['color'] }}-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-4">
                <span class="inline-flex items-center px-3 py-1 text-sm font-medium rounded-full bg-{{ $permissions['role_display']['color'] }}-100 text-{{ $permissions['role_display']['color'] }}-800">
                    {{ $permissions['role_display']['label'] }}
                </span>
                @if($user->is_active)
            ...
            <div class="space-y-2 text-sm">
                ...
                @if($permissions['permissions']['is_agent'])
                ...
                @if($permissions['permissions']['view_scraping_metrics'])
                ...
                @if(!$permissions['is_system_accessible'])
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <span class="text-gray-900">{{ $lastLogin['formatted'] }}</span>
                    @if($lastLogin['relative'] !== 'Never')
                        <span class="block text-xs text-gray-500">{{ $lastLogin['relative'] }}</span>
                    @endif
                </div>
                ...
                @if($lastLogin['ip'])
                    <div>
                        ...
                        <span class="text-gray-600 font-mono text-xs">{{ $lastLogin['ip'] }}</span>
                    </div>
                @endif
                ...
                @if($lastLogin['user_agent'])
                    <div>
                        ...
                        <span class="text-gray-600 text-xs">{{ Str::limit($lastLogin['user_agent'], 60) }}</span>
                    </div>
                @endif

                <div>
                    <label class="block font-medium text-gray-700">Email Verified</label>
                    @if($activityStats['email_verified'])
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <span class="text-2xl font-bold text-purple-600">{{ number_format($activityStats['login_count']) }}</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Activity Score</label>
                    <span class="text-2xl font-bold text-indigo-600">{{ number_format($activityStats['activity_score']) }}</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Account Age</label>
                    <span class="text-gray-900">{{ $activityStats['account_age_days'] }} days</span>
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Last Activity</label>
                    <span class="text-gray-600 text-xs">{{ $activityStats['last_activity'] }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">
                        {{ $accountCreation['source_label'] }}
                    </span>
                </div>

                <div>
                    <label class="block font-medium text-gray-700">Created</label>
                    <span class="text-gray-900">{{ $accountCreation['created_at_formatted'] }}</span>
                    <span class="block text-xs text-gray-500">{{ $accountCreation['created_at_relative'] }}</span>
                </div>

                <div>
                    <label class="block font-medium text-gray-700">Created By</label>
                    <span class="text-gray-600">
                        @if($accountCreation['created_by_type'] === 'self')
                            Self Registration
                        @elseif($accountCreation['created_by_type'] === 'admin')
                            Administrator
                        @elseif($accountCreation['created_by_type'] === 'system')
                            System Generated
                        @else
                            {{ ucfirst($accountCreation['created_by_type']) }}
                        @endif
                    </span>
                </div>
            </div>
