...
<div class="touch-filter-controls {{ $position }} top-0 left-0 right-0 bg-white border-b border-gray-200 z-30"
  data-filter-id="{{ $filterId }}" data-position="{{ $position }}">
  <!-- Filter Header -->
  <div class="flex items-center justify-between px-4 py-3 md:px-6">
    ...
    <div class="flex items-center space-x-2">
      ...
      <button
        ...
        data-toggle-advanced="{{ $filterId }}" aria-label="Toggle advanced filters">
      ...
      <div class="filter-group mb-4">
        ...
        <div class="flex flex-wrap gap-2">
          @forelse($sportCategories as $category)
            @php
              $logoInfo = $leagueLogos[$category->name] ?? [
                  'emoji' => '🏆',
            ...
            <button
              ...
              data-filter-type="sport" data-filter-value="{{ $category->slug ?? $category->name }}"
              data-filter-label="{{ $category->name }}" aria-pressed="{{ $isActive ? 'true' : 'false' }}">
        ...
        <div class="flex flex-wrap gap-2">
          @foreach ($priceRangeOptions as $range)
            ...
            <button
              ...
              data-filter-type="price" data-filter-value="{{ $range['value'] }}"
              data-filter-label="{{ $range['label'] }}" aria-pressed="{{ $isActive ? 'true' : 'false' }}">
        ...
        <div class="flex flex-wrap gap-2">
          @foreach ($dateRangeOptions as $range)
            ...
            <button
              ...
              data-filter-type="date" data-filter-value="{{ $range['value'] }}"
              data-filter-label="{{ $range['label'] }}" aria-pressed="{{ $isActive ? 'true' : 'false' }}">
              <span class="text-lg mr-2">{{ $range['icon'] }}</span>
              <span>{{ $range['label'] }}</span>
            </button>
        ...
        <div class="filter-group">
          ...
          <div class="relative">
            <select
              ...
              data-filter-type="venue" multiple>
              <option value="">All Venues</option>
              @foreach ($venues as $venue)
                <option value="{{ $venue->slug ?? $venue->name }}"
                  {{ in_array($venue->slug ?? $venue->name, $activeFilters) ? 'selected' : '' }}>
                  {{ $venue->name }} @if (isset($venue->city))
                    ({{ $venue->city }})
                  @endif
                </option>
              @endforeach
        ...
        <div class="flex items-center space-x-3">
          <div class="flex-1">
            ...
            <input type="number"
              ...
              placeholder="$0" min="0" data-filter-type="price_min">
          </div>
          <div class="flex-1">
            ...
            <input type="number"
              ...
              placeholder="$1000" min="0" data-filter-type="price_max">
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          @foreach ($availabilityOptions as $option)
            ...
            <button
              ...
              data-filter-type="availability" data-filter-value="{{ $option['value'] }}"
              data-filter-label="{{ $option['label'] }}" aria-pressed="{{ $isActive ? 'true' : 'false' }}">
