...
<div class="responsive-data-table" data-table-id="{{ $tableId }}" data-card-layout="{{ $cardLayout }}"
  data-sortable="{{ $sortable ? 'true' : 'false' }}">
  <!-- Table Header with Controls -->
  @if ($showFilters || $showSearch)
    <div
      class="table-controls bg-white border border-gray-200 rounded-t-lg p-4 space-y-4 md:space-y-0 md:flex md:items-center md:justify-between">
      <!-- Search -->
      @if ($showSearch)
      ...
      <div class="flex items-center space-x-3">
        ...
        <div class="view-toggle md:hidden">
          <button
            ...
            data-toggle-view="{{ $tableId }}" aria-label="Toggle view">
  ...
  <div
    class="table-container relative bg-white {{ $showFilters || $showSearch ? 'border-l border-r border-b border-gray-200 rounded-b-lg' : 'border border-gray-200 rounded-lg' }}">
    <!-- Loading Overlay -->
    ...
    @if ($hasData)
              ...
              <tr>
                @foreach ($columns as $column)
                  ...
                  <th
                    ...
                    @if ($sortable) data-sortable-column="{{ $column['key'] ?? '' }}"
                                            data-current-direction="{{ $currentSort ? $sortDirection : '' }}" @endif>
                    <div class="flex items-center space-x-1">
                      <span>{{ $column['label'] }}</span>
                      @if ($sortable)
                        <div class="sort-indicators">
                          <svg
                            class="w-4 h-4 {{ $currentSort && $sortDirection === 'asc' ? 'text-blue-600' : 'text-gray-300' }}"
                      ...
                      <div class="flex items-center justify-end space-x-2">
                        @foreach ($actionButtons as $action)
                          <button
                            ...
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="{{ $action['icon'] ?? 'M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z' }}">
                              </path>
                            </svg>
                          </button>
                        @endforeach
      ...
      <div class="card-view block md:hidden" data-view="cards">
        <div class="space-y-4 p-4">
          @foreach ($data as $row)
            <div
              class="ticket-card bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow"
              data-row-id="{{ $row->id ?? '' }}">
        ...
        <div class="hidden md:flex md:items-center md:justify-between w-full">
          <div>
            <p class="text-sm text-gray-700">
              ...
              {{ min($currentPage * $itemsPerPage, $totalItems) }} of {{ $totalItems }} results
            </p>
          </div>
