...
@php
  ...
  $navItems = [
      [
          ...
          'route' => 'dashboard',
          'icon' =>
              'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a1 1 0 001 1h1a1 1 0 001-1V7a2 2 0 00-2-2H5zM5 14a2 2 0 00-2 2v3a1 1 0 001 1h1a1 1 0 001-1v-3a2 2 0 00-2-2H5z',
          'emoji' => '🎫',
      ],
      [
          'id' => 'categories',
      ...
      [
          ...
          'icon' =>
              'M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17M17 13a2 2 0 100 4 2 2 0 000-4zm-8 4a2 2 0 11-4 0 2 2 0 014 0z',
          'emoji' => '🛒',
          'badge' => true,
          'count' => $cartCount,
      ],
...
<div class="mobile-bottom-nav fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 md:hidden"
  ...
  <nav class="flex items-center justify-around px-2 py-2" role="tablist">
    @foreach ($navItems as $item)
      ...
      <a href="{{ $href }}"
        ...
        role="tab" aria-selected="{{ $isActive ? 'true' : 'false' }}" aria-label="{{ $item['label'] }}"
        data-nav-item="{{ $item['id'] }}" @if (!$routeExists) onclick="return false;" @endif>
        <!-- Icon Container -->
        <div class="relative icon-container mb-1">
          <!-- SVG Icon (Primary) -->
          <svg class="nav-icon w-6 h-6 transition-all duration-300 {{ $isActive ? 'text-blue-600' : 'text-gray-500' }}"
          ...
          @if ($showBadge || ($item['id'] === 'favorites' && $showNotificationBadge && $notificationCount > 0))
            <div
              class="absolute -top-2 -right-2 min-w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center animate-pulse">
          ...
          <!-- Active Indicator -->
          <div
            class="active-indicator absolute inset-0 bg-blue-500 bg-opacity-10 rounded-full scale-0 transition-transform duration-300">
          </div>
        </div>

        <!-- Label -->
        <span
          class="nav-label text-xs font-medium transition-colors duration-300 {{ $isActive ? 'text-blue-600' : 'text-gray-500' }}">
          {{ $item['label'] }}
        </span>
  ...
  <div class="quick-action-fab absolute -top-6 left-1/2 transform -translate-x-1/2">
    <button
      ...
      data-quick-action="search" aria-label="Quick search" title="Search tickets">
...
@push('styles')
  ...
  <style>
    ...
    @media screen and (orientation: landscape) and (max-height: 500px) {
      ...
      .quick-action-fab {
        display: none;
        /* Hide FAB in landscape to save space */
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function openQuickSearch() {
        ...
        searchOverlay.className =
          'quick-search-overlay fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end md:items-center md:justify-center';

        const searchContainer = document.createElement('div');
        searchContainer.className =
          'bg-white w-full max-w-md mx-auto rounded-t-xl md:rounded-xl p-6 transform transition-transform duration-300 translate-y-full';
      ...
      function showContextMenu(item, itemId) {
        ...
        menu.className =
          'context-menu absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-white rounded-lg shadow-xl border p-2 z-50 min-w-32';
      ...
      function showToast(message, type = 'info') {
        ...
        toast.className =
          `fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-4 py-2 rounded-lg text-white font-medium ${getToastColor(type)} animate-fade-in-scale`;
