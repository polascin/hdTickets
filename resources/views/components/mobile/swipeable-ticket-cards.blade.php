...
<div class="swipeable-ticket-cards" data-container-id="{{ $containerId }}">
  ...
  <!-- Tickets Grid -->
  <div class="tickets-grid {{ $enableSwipe ? 'swipe-enabled' : '' }}" data-tickets-grid="{{ $containerId }}"
    data-card-layout="{{ $cardLayout }}"
    @if ($enableSwipe) data-swipe-container="true" 
            data-swipe-threshold="50" @endif>
    @forelse($tickets as $index => $ticket)
      <div class="ticket-card-wrapper relative {{ $cardLayout === 'minimal' ? 'mb-2' : 'mb-4' }}"
        data-ticket-id="{{ $ticket->id }}" data-card-index="{{ $index }}"
        @if ($enableSwipe) data-swipe-item="true"
                    data-swipe-actions='["favorite", "purchase", "share"]' @endif>
        <!-- Swipe Action Indicators -->
        @if ($enableSwipe)
          <div
            class="swipe-actions absolute inset-0 flex items-center justify-between px-4 opacity-0 pointer-events-none z-10">
            <!-- Left Swipe Action (Favorite) -->
            <div
              class="swipe-action-left flex items-center justify-center w-16 h-16 bg-yellow-500 text-white rounded-full transform scale-75">
            ...
            <div class="swipe-actions-right flex space-x-2">
              <div
                class="swipe-action-purchase flex items-center justify-center w-16 h-16 bg-green-500 text-white rounded-full transform scale-75">
              ...
              <div
                class="swipe-action-share flex items-center justify-center w-16 h-16 bg-blue-500 text-white rounded-full transform scale-75">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Ticket Card -->
        <div
          class="ticket-card bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 {{ $cardLayout === 'minimal' ? 'p-3' : 'p-4' }}">
            ...
            <div class="flex flex-col h-full">
              ...
              <div class="flex items-start justify-between mb-3">
                ...
                <div class="ml-3">
                  ...
                  @endphp
                  <span
                    class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $availabilityColors[$availability] ?? 'bg-gray-100 text-gray-800' }}">
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-2">
                  <button class="favorite-btn p-2 rounded-full hover:bg-gray-100 transition-colors"
                    ...
                    <svg class="w-5 h-5 text-gray-400 hover:text-yellow-500" fill="none" stroke="currentColor"
                      viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  @if ($availability !== 'sold_out')
                    <button
                      class="buy-btn bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors text-sm">
                      Buy Now
                    </button>
                  @endif
                </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500">
                  {{ \Carbon\Carbon::parse($ticket->event_date ?? $ticket->date)->format('M j') }}</p>
              </div>
              <div class="ml-3 text-right">
                <div class="text-lg font-bold text-green-600">
                  ${{ number_format($ticket->price ?? $ticket->current_price, 0) }}</div>
          ...
          @else
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-start justify-between">
                ...
                <span
                  class="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $availabilityColors[$availability] ?? 'bg-gray-100 text-gray-800' }}">
                  {{ ucfirst(str_replace('_', ' ', $availability)) }}
                </span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                @if ($availability !== 'sold_out')
                  <button
                    class="buy-btn bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg font-medium transition-colors text-sm">
                    Buy
                  </button>
                @endif
              </div>
        ...
        <div
          class="quick-actions absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    @empty
      ...
      <div class="col-span-full text-center py-12">
        ...
        <p class="text-gray-500 max-w-sm mx-auto">There are no sports event tickets matching your criteria. Try
  ...
  @if ($showPagination && count($tickets) > 0)
    <div class="load-more-container mt-6 text-center">
      ...
      @else
        <button
          class="load-more-btn bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-3 rounded-lg font-medium transition-colors">
          Load More Tickets
        </button>
      @endif
    </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function showSwipeActionFeedback(message, type) {
        ...
        feedback.className =
          `fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-4 py-2 rounded-lg text-white font-medium ${type === 'success' ? 'bg-green-500' : 'bg-red-500'} animate-fade-in-scale`;
