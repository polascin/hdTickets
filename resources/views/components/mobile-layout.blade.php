...
<div class="mobile-layout min-h-screen bg-gray-50 relative" data-mobile-layout
  @if ($pullToRefresh) data-pull-to-refresh @endif
  @if ($swipeGestures) data-swipe-gestures @endif>

  <!-- Mobile Header -->
  <header
    class="mobile-header bg-white shadow-sm border-b border-gray-200 {{ $fixedHeader ? 'fixed top-0 left-0 right-0 z-50' : '' }}">
    <div class="flex items-center justify-between px-4 py-3 h-16">
      ...
      <div class="flex items-center space-x-2">
        @if ($showSearch)
          <button
            class="mobile-search-toggle touch-target p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if ($showUserMenu)
          <!-- User Menu Button -->
          <button
            class="mobile-user-menu-toggle touch-target p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
            @php
              $profileDisplay = Auth::user()->getProfileDisplay();
            @endphp
            @if ($profileDisplay['has_picture'])
              <img class="w-6 h-6 rounded-full object-cover" src="{{ $profileDisplay['picture_url'] }}"
  ...
  @if ($pullToRefresh)
    <div
      class="pull-to-refresh-indicator fixed top-16 left-1/2 transform -translate-x-1/2 -translate-y-full transition-transform duration-200 z-40">
  ...
  @if ($showUserMenu)
    <div class="mobile-user-menu fixed inset-0 z-50 hidden">
      ...
      <!-- Menu Panel -->
      <div
        class="fixed bottom-0 left-0 right-0 bg-white rounded-t-xl shadow-xl transform transition-transform translate-y-full">
        <div class="p-6">
          <!-- User Info -->
          <div class="flex items-center space-x-4 mb-6">
            @if ($profileDisplay['has_picture'])
              <img class="w-12 h-12 rounded-full object-cover" src="{{ $profileDisplay['picture_url'] }}"
                alt="{{ $profileDisplay['display_name'] }}">
            @else
