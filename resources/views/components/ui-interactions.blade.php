        ...
        >
            ...
            <div class="mt-4 w-full bg-gray-200 rounded-full h-1">
                <div 
                    ...
                    :style="{ width: ((tour.stepIndex + 1) / tour.steps.length) * 100 + '%' }"
                ></div>
            </div>
        </div>
    </div>
...
<style>
    ...
    .keyboard-shortcut {
        @apply inline-flex items-center px-2 py-1 bg-gray-100 border border-gray-300 rounded text-xs font-mono text-gray-600;
    }
</style>
...
function uiInteractions() {
    return {
        ...
        getContextMenuItems(menuType) {
            const menus = {
                ticket: [
                    { id: 'view', label: 'View Details', icon: '👁️', action: 'viewTicket' },
                    { id: 'favorite', label: 'Add to Favorites', icon: '❤️', action: 'favoriteTicket' },
                    { id: 'alert', label: 'Create Price Alert', icon: '🔔', action: 'createAlert' },
                    { id: 'share', label: 'Share', icon: '📤', action: 'shareTicket', separator: true },
                    { id: 'hide', label: 'Hide', icon: '🙈', action: 'hideTicket', danger: true }
                ],
                dashboard: [
                    { id: 'refresh', label: 'Refresh', icon: '🔄', action: 'refresh', shortcut: 'Ctrl+R' },
                    { id: 'export', label: 'Export Data', icon: '📊', action: 'exportData' },
        ...
        applyValidationRule(value, rule, ruleValue) {
            ...
            return validator ? validator(ruleValue) : { valid: true, message: '' };
        },
