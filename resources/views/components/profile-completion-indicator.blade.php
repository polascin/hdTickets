...
<div class="profile-completion-indicator flex items-center space-x-2" x-data="{ showTooltip: false }"
    ...
    <div class="relative {{ $sizes['progress'] }}">
        ...
        <svg class="absolute inset-0 transform -rotate-90 {{ $sizes['progress'] }}" viewBox="0 0 36 36">
            <path class="{{ str_replace('bg-', 'text-', explode(' ', $config['color'])[1]) }}" stroke="currentColor"
                stroke-width="3" fill="none" stroke-dasharray="{{ $completion['percentage'] }}, 100"
                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
        </svg>
        ...
        <div class="absolute inset-0 flex items-center justify-center">
            @if ($completion['percentage'] >= 90)
                ...
                <svg class="{{ $sizes['icon'] }} {{ explode(' ', $config['color'])[0] }}" fill="currentColor"
                    viewBox="0 0 20 20">
        ...
        <div class="flex flex-col">
            <div class="flex items-center space-x-1">
                ...
                @if ($position === 'dropdown')
                    <span
                        class="inline-flex items-center {{ $sizes['badge'] }} {{ $config['color'] }} rounded-full font-medium">
                        {{ $config['text'] }}
                    </span>
                @endif
            </div>
            @if ($position === 'sidebar')
                <span
                    class="text-xs text-gray-500">{{ $completion['completed_count'] }}/{{ $completion['total_fields'] }}
                    completed</span>
            @endif
        </div>
    @endif
