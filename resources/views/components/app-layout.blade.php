...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
  ...
  <head>
    ...
    <title>{{ config('app.name', 'HD Tickets') }} - @isset($title)
        {{ $title }}
      @else
        Dashboard
      @endisset
        ...
        if (passwordInput && toggleButton) {
          ...
          if (toggleDescription) {
            toggleDescription.textContent =
              `Click to toggle password visibility. Current state: ${isPassword ? 'visible' : 'hidden'}`;
          }
          ...
          if (passwordIcon) {
            if (isPassword) {
              ...
              passwordIcon.innerHTML =
                '<title>Hide password</title><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>';
            } else {
              // Eye-slash icon (hidden)
              passwordIcon.innerHTML =
                '<title>Show password</title><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L12 12m-3.122-3.122l4.243 4.243M12 12l6.12 6.12M12 12L9.88 9.88m8.242 8.242L21 21M12 12l2.88-2.88"></path>';
            }
          }
