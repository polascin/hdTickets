...
@push('styles')
    ...
    <style>
        /* Override any container constraints for full dashboard experience */
        body {
        ...
        .enhanced-customer-dashboard .dashboard-header {
        ...
        .enhanced-customer-dashboard img {
            max-width: 100%;
            height: auto;
        }

        /* Alpine.js cloak for smooth loading */
        [x-cloak] {
            ...
            <div class="header-content">
                    ...
                    <div class="stats-summary">
                        <div class="stat-item">
                            <span class="stat-value"
                                x-text="formatNumber(dashboardData.statistics?.available_tickets?.current || 0)">
                                {{ number_format($statistics['available_tickets']['current'] ?? 0) }}
                            </span>
                            <span class="stat-label">Available</span>
                            <div class="trend-indicator"
                                :class="getTrendClass(dashboardData.statistics?.available_tickets?.trend)">
                ...
                <div class="header-actions">
                    <div class="refresh-controls">
                        <button @click="toggleAutoRefresh()" :class="{ 'active': autoRefresh }" class="auto-refresh-btn"
                        ...
                        <button @click="refreshDashboard()" :disabled="isLoading" class="refresh-btn">
                            <svg :class="{ 'animate-spin': isLoading }" class="icon" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                    ...
                    <div class="action-buttons">
                        ...
                        <button @click="openNotifications()" class="notification-btn"
                            ...
                            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M15 17h5l-5 5v-5zM12 17H7a3 3 0 01-3-3V5a3 3 0 013-3h5" />
                            </svg>
                            <span class="notification-badge" x-show="notificationCount > 0"
            ...
            <section class="metrics-grid">
                    ...
                    <div class="metric-content">
                        ...
                        <div class="metric-value-container">
                            <span class="metric-value"
                                x-text="formatNumber(dashboardData.statistics?.available_tickets?.current || {{ $statistics['available_tickets']['current'] ?? 0 }})">
                                {{ number_format($statistics['available_tickets']['current'] ?? 0) }}
                            </span>
                ...
                <div class="metric-card high-demand" @click="navigateTo('{{ route('tickets.scraping.trending') }}')">
                    ...
                    <div class="metric-content">
                        ...
                        <div class="metric-value-container">
                            <span class="metric-value"
                                x-text="formatNumber(dashboardData.statistics?.high_demand?.current || {{ $statistics['high_demand']['current'] ?? 0 }})">
                ...
                <div class="metric-card alerts" @click="navigateTo('{{ route('tickets.alerts.index') }}')">
                    <div class="metric-header">
                        ...
                        <div class="alert-indicator"
                            x-show="dashboardData.statistics?.active_alerts?.triggered_today > 0">
                    ...
                    <div class="metric-content">
                        ...
                        <div class="metric-value-container">
                            <span class="metric-value"
                                x-text="dashboardData.statistics?.active_alerts?.current || {{ $statistics['active_alerts']['current'] ?? 0 }}">
                                {{ $statistics['active_alerts']['current'] ?? 0 }}
                            </span>
                        </div>
                        <p class="metric-description">
                            Active monitoring •
                            <span
                                x-text="(dashboardData.statistics?.active_alerts?.success_rate || {{ $statistics['active_alerts']['success_rate'] ?? 0 }}) + '%'">{{ $statistics['active_alerts']['success_rate'] ?? 0 }}%</span>
                            success rate
                        </p>
                    </div>
                ...
                <div class="metric-card user-activity">
                    <div class="metric-header">
                        ...
                        <div class="engagement-score">
                            <div class="score-circle">
                                <span
                                    x-text="Math.round(dashboardData.statistics?.user_activity?.engagement_score || {{ $statistics['user_activity']['engagement_score'] ?? 75 }})">
                    ...
                    <div class="metric-content">
                        ...
                        <div class="metric-value-container">
                            <span class="metric-value"
                                x-text="dashboardData.statistics?.user_activity?.views_today || {{ $statistics['user_activity']['views_today'] ?? 0 }}">
                ...
                <div class="actions-grid">
                    <a href="{{ route('tickets.scraping.index') }}" class="action-card browse-tickets">
                        ...
                        <div class="action-content">
                            ...
                            <span
                                class="action-badge">{{ number_format($statistics['available_tickets']['current'] ?? 0) }}
                                available</span>
                        </div>
                    </a>
                    ...
                    <div class="widget-content">
                        ...
                        @if (isset($recentTickets) && $recentTickets->isNotEmpty())
                            @foreach ($recentTickets as $ticket)
                                <div class="ticket-item"
                                    onclick="window.open('{{ route('tickets.scraping.show', $ticket->id) }}', '_blank')">
                                    <div class="ticket-header">
                                    ...
                                    <div class="ticket-details">
                                        ...
                                        <span
                                            class="ticket-date">{{ $ticket->event_date ? \Carbon\Carbon::parse($ticket->event_date)->format('M j, Y') : 'TBD' }}</span>
                                    </div>
                                    <div class="ticket-indicators">
                                        <span
                                            class="demand-indicator demand-{{ strtolower($ticket->demand_indicator ?? 'medium') }}">
                        ...
                        @else
                            <div class="empty-state">
                                ...
                                <a href="{{ route('tickets.scraping.index') }}" class="empty-action">Browse Available
                                    Tickets</a>
                            </div>
                        @endif
                    </div>
                            ...
                            @foreach ($personalizedRecommendations as $recommendation)
                                <div class="recommendation-item">
                                    <div class="recommendation-header">
                                        ...
                                        <div class="confidence-score">{{ round($recommendation['confidence_score']) }}%
                                        </div>
                                    </div>
                                    <div class="recommendation-details">
                                        <span
                                            class="recommendation-price">${{ number_format($recommendation['ticket']->price, 2) }}</span>
                    ...
                    <div class="widget-content">
                        ...
                        <div class="trend-summary">
                            ...
                            <div class="trend-item">
                                ...
                                <span
                                    class="trend-value">{{ number_format($statistics['available_tickets']['current'] ?? 0) }}</span>
                            ...
                            @foreach ($upcomingEvents->take(4) as $event)
                                <div class="event-item">
                                    <div class="event-date">
                                        <span
                                            class="month">{{ \Carbon\Carbon::parse($event->event_date)->format('M') }}</span>
                                        <span
                                            class="day">{{ \Carbon\Carbon::parse($event->event_date)->format('j') }}</span>
                                    </div>
