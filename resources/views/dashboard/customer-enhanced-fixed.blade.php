...
@push('styles')
    ...
    <style>
        /* Improved layout fixes with better fallbacks */
        body {
        ...
        /* Alpine.js cloak for smoother loading */
        [x-cloak] {
    ...
    <div class="enhanced-customer-dashboard" 
            ...
            <div class="header-content">
                <div class="header-info">
                    ...
                    <div class="stats-summary">
                        <div class="stat-item">
                            <span class="stat-value" x-text="formatNumber(dashboardData.statistics?.available_tickets?.current || {{ $statistics['available_tickets']['current'] ?? 0 }})">
                        ...
                        <div class="stat-item">
                            <span class="stat-value" x-text="dashboardData.statistics?.active_alerts?.current || {{ $statistics['active_alerts']['current'] ?? 0 }}">
                        ...
                        <div class="stat-item">
                            <span class="stat-value" x-text="dashboardData.statistics?.high_demand?.current || {{ $statistics['high_demand']['current'] ?? 0 }}">
        ...
        <main class="dashboard-main">
            ...
            <section class="metrics-grid">
                ...
                <div class="metric-card available-tickets" 
                     @click="navigateTo('{{ route('tickets.scraping.index') }}')"
                     role="button"
                     tabindex="0"
                     @keydown.enter="navigateTo('{{ route('tickets.scraping.index') }}')">
                    ...
                    <div class="metric-content">
                        ...
                        <div class="metric-value-container">
                            ...
                            <span class="metric-change" 
                                  x-show="dashboardData.statistics?.available_tickets?.change_24h" 
                                  x-text="formatChange(dashboardData.statistics?.available_tickets?.change_24h)">
                            </span>
                        </div>
                        <p class="metric-description">Active sports ticket listings</p>
                    </div>
                ...
                <!-- High Demand Card -->
                <div class="metric-card high-demand" 
                     @click="navigateTo('{{ route('tickets.scraping.trending') }}')"
                     role="button"
                     tabindex="0"
                     @keydown.enter="navigateTo('{{ route('tickets.scraping.trending') }}')">
                    <div class="metric-header">
                    ...
                    <div class="metric-content">
                        ...
                        <p class="metric-description">Popular sports events</p>
                    </div>
                ...
                <div class="metric-card alerts" 
                     @click="navigateTo('{{ route('tickets.alerts.index') }}')"
                     role="button"
                     tabindex="0"
                     @keydown.enter="navigateTo('{{ route('tickets.alerts.index') }}')">
                    <div class="metric-header">
                    ...
                    <div class="metric-content">
                        ...
                        <p class="metric-description">
                            Active monitoring • 
                            <span x-text="(dashboardData.statistics?.active_alerts?.success_rate || {{ $statistics['active_alerts']['success_rate'] ?? 0 }}) + '%'">{{ $statistics['active_alerts']['success_rate'] ?? 0 }}%</span> 
                            success rate
                        </p>
                    </div>
            ...
            <section class="quick-actions">
                <div class="section-header">
                    ...
                    <p class="section-subtitle">Essential sports ticket management tools</p>
                </div>
                ...
                <div class="actions-grid">
                    ...
                    <a href="{{ route('tickets.alerts.index') }}" class="action-card create-alert">
                    ...
                    <div class="action-card settings" @click="openSettings()" role="button" tabindex="0" @keydown.enter="openSettings()">
                ...
                <section class="dashboard-widget recent-tickets">
                    ...
                    <div class="widget-content">
                        ...
                        <template x-for="ticket in recentTickets" :key="ticket.id">
                            <div class="ticket-item" @click="viewTicket(ticket)" role="button" tabindex="0" @keydown.enter="viewTicket(ticket)">
                        ...
                        @if (isset($recentTickets) && $recentTickets->isNotEmpty())
                            @foreach ($recentTickets as $ticket)
                                <div class="ticket-item" onclick="window.open('{{ route('tickets.scraping.show', $ticket['id'] ?? $ticket->id) }}', '_blank')">
                                    <div class="ticket-header">
                                        <div class="ticket-title">{{ $ticket['title'] ?? $ticket->title ?? 'Sports Event' }}</div>
                                        <div class="ticket-price">${{ number_format($ticket['price'] ?? $ticket->min_price ?? 0, 2) }}</div>
                                    </div>
                                    <div class="ticket-details">
                                        <span class="ticket-platform">{{ $ticket['platform'] ?? $ticket->platform ?? 'Platform' }}</span>
                                        <span class="ticket-venue">{{ $ticket['venue'] ?? $ticket->venue ?? 'Venue TBD' }}</span>
                                        <span class="ticket-date">
                                            @if (isset($ticket['event_date']) && $ticket['event_date'])
                                                {{ \Carbon\Carbon::parse($ticket['event_date'])->format('M j, Y') }}
                                            @elseif (isset($ticket->event_date) && $ticket->event_date)
                                                {{ \Carbon\Carbon::parse($ticket->event_date)->format('M j, Y') }}
                                            @else
                                                TBD
                                            @endif
                                    ...
                                    <div class="ticket-indicators">
                                        <span class="demand-indicator demand-{{ strtolower($ticket['demand_indicator'] ?? $ticket->demand_indicator ?? 'medium') }}">
                                            {{ ucfirst($ticket['demand_indicator'] ?? $ticket->demand_indicator ?? 'medium') }}
                                        </span>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            @foreach ($personalizedRecommendations as $recommendation)
                                <div class="recommendation-item">
                                    ...
                                    <div class="recommendation-details">
                                        <span class="recommendation-price">${{ number_format($recommendation['ticket']->min_price ?? 0, 2) }}</span>
                    ...
                    <div class="widget-content">
                        <div class="chart-container">
                            ...
                            <div id="chartFallback" class="chart-fallback">
                                📈 Chart will load when data is available
                            </div>
                        </div>
                            ...
                            @foreach ($upcomingEvents->take(4) as $event)
                                <div class="event-item">
                                    ...
                                    <div class="event-details">
                                        ...
                                        <div class="event-price">From ${{ number_format($event->min_price ?? 0, 2) }}</div>
                                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        // Error handling for missing Alpine.js
        if (typeof Alpine === 'undefined') {
            console.warn('Alpine.js not loaded, using fallback functionality');
            
            // Basic fallback functionality
            window.enhancedCustomerDashboard = function() {
                return {
                    isLoading: false,
                    error: null,
                    autoRefresh: false,
                    ...
                    showSettings: false,
                    dashboardData: window.dashboardInitialData || {},
                    recentTickets: window.dashboardInitialData?.recentTickets || [],
                    notifications: [],
                    settings: {
                        autoRefresh: false,
                        refreshInterval: 5,
                    ...
                    init() {
                        console.log('Dashboard fallback initialized');
                        this.loadInitialData();
                    },
                    
                    loadInitialData() {
                        if (window.dashboardInitialData) {
                            this.dashboardData = window.dashboardInitialData;
                        }
                    },
