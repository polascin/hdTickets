<x-app-layout>
    <div class="min-h-screen bg-gray-50" x-data="mainDashboard()">
                                    ...
                                    <div class="flex items-start justify-between mb-3">
                                        ...
                                        <div class="text-right">
                                            ...
                                            <div x-show="ticket.original_price && ticket.original_price > ticket.price" 
                                                 ...
                                                 x-text="'$' + ticket.original_price"></div>
                                        </div>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span :class="`px-2 py-1 rounded-full text-xs font-medium ${ticket.status === 'available' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`"
                        ...
                        <div class="space-y-4">
                            <template x-for="activity in recentActivity" :key="activity.id">
                                <div class="flex items-start space-x-3">
                                    <div class="flex-shrink-0">
                                        <div :class="`w-8 h-8 rounded-full flex items-center justify-center ${activity.type === 'purchase' ? 'bg-green-100' : activity.type === 'alert' ? 'bg-yellow-100' : 'bg-blue-100'}`">
                                            <svg class="w-4 h-4" :class="`${activity.type === 'purchase' ? 'text-green-600' : activity.type === 'alert' ? 'text-yellow-600' : 'text-blue-600'}`" 
                                                 fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path x-show="activity.type === 'purchase'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
                                                <path x-show="activity.type === 'alert'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM4 19l1-7H3l1 7zM15 3H4l11 4v12z"/>
                                                <path x-show="activity.type === 'watchlist'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                                            </svg>
                                        </div>
                                    </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                                            <div class="bg-indigo-600 h-2 rounded-full transition-all duration-300" style="width: {{ min($percentage, 100) }}%"></div>
                                        </div>
                                    </div>
        ...
        function mainDashboard() {
            return {
                ...
                addToWatchlist(ticketId) {
                    this.makeRequest('POST', '/api/v1/watchlist', { ticket_id: ticketId })
                        .then(data => {
                            if (data.success) {
