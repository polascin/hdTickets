...
@section('content')
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Scraper Dashboard</h1>
      <p class="text-sm text-slate-500 dark:text-slate-400">Ticket Scraping & Monitoring Control Panel</p>
    </div>
    <div class="flex items-center space-x-4">
      <div class="text-sm text-slate-600 dark:text-slate-300">
        Last Sync: <span class="text-green-600 font-semibold">{{ now()->format('H:i:s') }}</span>
      </div>
      <button onclick="refreshScraperDashboard()" class="uiv2-action-btn bg-green-600 hover:bg-green-700 text-white">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <x-dashboard.stat-card title="Tickets Scraped Today"
    ...
    <x-dashboard.stat-card title="Active Scraping Jobs" value="{{ $scraperMetrics['active_scraping_jobs'] }}"
    ...
    <x-dashboard.stat-card title="Success Rate" value="{{ $scraperMetrics['success_rate'] }}"
    ...
    <x-dashboard.stat-card title="Data Quality Score" value="{{ $scraperMetrics['data_quality_score'] }}"
          ...
          <div class="space-y-4">
            @foreach ($scrapingJobs['active_jobs'] as $job)
              <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div class="ml-4">
                  <span
                    class="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">{{ ucfirst($job['status']) }}</span>
                </div>
              </div>
            @endforeach
          </div>
              ...
              <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <h5 class="text-sm font-medium text-gray-900 capitalize">
                    {{ ucfirst(str_replace('_', ' ', $platform)) }}</h5>
                  <p class="text-xs text-gray-500">Response: {{ $status['response_time'] }} • Success:
                    {{ $status['success_rate'] }}</p>
                </div>
                <div class="flex items-center">
                  <div
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div
      class="bg-gradient-to-br from-green-50 to-emerald-100 border border-green-200 p-6 rounded-xl hover:shadow-lg transition cursor-pointer"
    ...
    <div
      ...
      onclick="window.location.href='{{ route('admin.scraping.index') }}'">
      <div class="flex items-center">
    ...
    <div
      class="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 p-6 rounded-xl hover:shadow-lg transition cursor-pointer">
      <div class="flex items-center">
            ...
            @foreach (array_slice($recentActivity, 0, 8) as $activity)
              <div class="flex items-start space-x-3">
                <div class="flex-shrink-0">
                  <div
                    class="w-8 h-8 rounded-full flex items-center justify-center {{ $activity['color'] === 'green' ? 'bg-green-100' : ($activity['color'] === 'red' ? 'bg-red-100' : 'bg-blue-100') }}">
                    <svg
                      class="w-4 h-4 {{ $activity['color'] === 'green' ? 'text-green-600' : ($activity['color'] === 'red' ? 'text-red-600' : 'text-blue-600') }}"
                      ...
                      @elseif($activity['icon'] === 'alert')
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.664-.833-2.464 0L5.34 16.5c-.77.833.192 2.5 1.732 2.5z">
                        </path>
                      @else
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2z"></path>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          @foreach ($platformData['anti_detection_status'] as $technique => $data)
            <div class="bg-gray-50 rounded-lg p-4 text-center">
              <div
                class="w-3 h-3 {{ $data['status'] === 'active' ? 'bg-green-400' : ($data['status'] === 'monitoring' ? 'bg-yellow-400' : 'bg-red-400') }} rounded-full mx-auto mb-2">
              </div>
          ...
          if (data.success) {
            ...
            document.querySelectorAll('[data-platform-status]').forEach(element => {
              ...
              if (data.data.platform_status[platform]) {
                ...
                if (indicator) {
                  indicator.className =
                    `w-3 h-3 rounded-full ${status === 'online' ? 'bg-green-400' : status === 'slow' ? 'bg-yellow-400' : 'bg-red-400'}`;
                }
              }
            });
