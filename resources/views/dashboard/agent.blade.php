<x-dashboard.layout title="Agent Dashboard - Sports Events Tickets" subtitle="Ticket Monitoring & Purchase Management">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <x-dashboard.stat-card 
            ...
            value="{{ $agentMetrics['tickets_monitored'] }}" 
        ...
        <x-dashboard.stat-card 
            title="Active Alerts" 
            value="{{ $agentMetrics['active_alerts'] }}" 
        ...
        <x-dashboard.stat-card 
            ...
            value="{{ $agentMetrics['successful_purchases_today'] }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="p-6">
                @if(isset($purchaseData['pending_purchases']) && count($purchaseData['pending_purchases']) > 0)
                        ...
                        @foreach(array_slice($purchaseData['pending_purchases'], 0, 5) as $purchase)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500">{{ $purchase['platform'] ?? 'Platform' }} • ${{ $purchase['price'] ?? '0.00' }}</p>
                                </div>
                    ...
                    <div class="mt-4">
                        <a href="{{ route('purchase-decisions.index') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                        ...
                        @foreach(array_slice($recentActivity, 0, 5) as $activity)
                            <div class="flex items-start space-x-3">
                                <div class="flex-shrink-0">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center {{ $activity['color'] === 'green' ? 'bg-green-100' : ($activity['color'] === 'yellow' ? 'bg-yellow-100' : 'bg-blue-100') }}">
                                        <svg class="w-4 h-4 {{ $activity['color'] === 'green' ? 'text-green-600' : ($activity['color'] === 'yellow' ? 'text-yellow-600' : 'text-blue-600') }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            @if($activity['icon'] === 'shopping-cart')
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17M17 13v4a2 2 0 01-2 2H9a2 2 0 01-2-2v-4m8 0V9a2 2 0 00-2-2H9a2 2 0 00-2 2v4.01"></path>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 p-6 rounded-xl hover:shadow-lg transition cursor-pointer" onclick="window.location.href='{{ route('tickets.scraping.index') }}'">
            <div class="flex items-center">
        ...
        <div class="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 p-6 rounded-xl hover:shadow-lg transition cursor-pointer" onclick="window.location.href='{{ route('purchase-decisions.index') }}'">
            <div class="flex items-center">
        ...
        <div class="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 p-6 rounded-xl hover:shadow-lg transition cursor-pointer" onclick="window.location.href='{{ route('tickets.alerts.index') }}'">
            <div class="flex items-center">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <h4 class="text-lg font-semibold text-gray-900 flex items-center">
                <svg class="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.25-2.25L21 7.5l-2.25-2.25L16.5 7.5 21 12l-2.25 2.25L16.5 16.5 21 12"></path>
                </svg>
                Platform Status
            </h4>
