...
@section('meta_description', 'Sports ticket monitoring system administration dashboard with analytics, user management,
...
@section('content')
  <div class="space-y-8" x-data="adminDashboard()" x-init="init()">
    ...
    <div class="dashboard-stats-grid">
      <x-hdt.stat-card label="Total Users" x-text="systemStats.totalUsers.toLocaleString()" variant="primary"
        trend="up" trendValue="+23" trendLabel="this month">
        <x-slot:icon>
      ...
      <x-hdt.stat-card label="System Load" x-text="systemStats.cpuUsage + '%'" :variant="(systemStats . cpuUsage > 80 ? 'danger' : systemStats . cpuUsage > 60) ? 'warning' : 'success'"
      ...
      <x-hdt.stat-card label="Revenue Today" x-text="'$' + systemStats.dailyRevenue.toLocaleString()" variant="success"
        trend="up" trendValue="+12.5%" trendLabel="vs yesterday">
        <x-slot:icon>
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <x-hdt.stat-card label="Active Scrapers" x-text="systemStats.activeScrapers + ' / ' + systemStats.totalScrapers"
        :variant="systemStats.scraperHealth === 'healthy' ? 'success' : systemStats.scraperHealth === 'warning' ? 'warning' : 'danger'" description="Scraping operations">
        <x-slot:icon>
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div class="xl:col-span-2 space-y-6">
        ...
        <x-hdt.card>
          ...
          <div class="h-80 relative">
            ...
            <canvas id="revenueChart" x-show="chartsLoaded" x-transition:enter="transition-opacity duration-300"
              x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="w-full h-full"></canvas>
          </div>
        </x-hdt.card>
        ...
        <x-hdt.card>
          ...
          <div class="h-64 relative">
            ...
            <canvas id="userGrowthChart" x-show="chartsLoaded" x-transition class="w-full h-full"></canvas>
          </div>
        </x-hdt.card>
                ...
                <div class="flex items-center justify-between p-3 bg-hd-gray-50 dark:bg-hd-gray-800 rounded-lg">
                  <div class="flex items-center gap-3">
                    <div class="w-3 h-3 rounded-full"
                      :class="server.status === 'online' ? 'bg-green-500' : server.status === 'warning' ? 'bg-yellow-500' :
                          'bg-red-500'">
                    </div>
                    <span class="text-sm font-medium" x-text="server.name"></span>
                  </div>
                  <x-hdt.badge size="xs" :variant="server.status === 'online' ? 'success' : server.status === 'warning' ? 'warning' : 'danger'" x-text="server.status"
              ...
              <div class="flex items-start gap-4 p-4 border border-hd-gray-200 dark:border-hd-gray-700 rounded-lg">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <x-hdt.badge size="xs" :variant="event.type === 'success' ? 'success' : event.type === 'warning' ? 'warning' : event.type === 'error' ? 'danger' : 'info'" x-text="event.type" class="capitalize"></x-hdt.badge>
                  </div>
                ...
                <x-hdt.button size="xs" variant="ghost" x-show="event.actionable"
      ...
      <div class="space-y-6">
        ...
        <x-hdt.card>
          ...
          <x-slot:subtitle>
            <a href="/admin/users" class="text-hd-secondary-600 hover:text-hd-secondary-700 text-sm font-medium">Manage
              All</a>
          </x-slot:subtitle>
            ...
            <div class="space-y-2" x-show="recentUsers.length > 0">
              <template x-for="user in recentUsers.slice(0, 3)" :key="user.id">
                <div class="flex items-center gap-3 p-2 bg-hd-gray-50 dark:bg-hd-gray-800 rounded">
                  <div
                    class="w-8 h-8 bg-hd-primary-600 text-white rounded-full flex items-center justify-center text-xs font-bold"
                  ...
                  <x-hdt.badge size="xs" :variant="user.role === 'customer' ? 'success' : user.role === 'agent' ? 'info' : 'warning'" x-text="user.role" class="capitalize"></x-hdt.badge>
                </div>
              </template>
            </div>
          </div>
        ...
        <x-hdt.card>
          ...
          <x-slot:subtitle>
            <x-hdt.badge size="sm" :variant="systemAlerts . length > 0 ? 'warning' : 'success'" x-text="systemAlerts.length + ' alerts'"></x-hdt.badge>
          </x-slot:subtitle>
            ...
            <template x-for="alert in systemAlerts" :key="alert.id">
              <div class="p-3 border rounded-lg"
                :class="{
                    ...
                    'bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800': alert
                        .severity === 'warning',
                    'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800': alert
                        .severity === 'info'
                }">
    ...
    function adminDashboard() {
      return {
        ...
        async fetchRevenueData() {
          ...
          const labels = [];
          const values = [];
          const days = this.revenueTimeframe === '7d' ? 7 : this.revenueTimeframe === '30d' ? 30 : this
            .revenueTimeframe === '90d' ? 90 : 365;
        ...
        updateSystemMetrics() {
          ...
          this.systemStats.memoryUsage = Math.max(30, Math.min(90, this.systemStats.memoryUsage + (Math.random() - 0.5) *
            5));
          this.systemStats.storageUsage = Math.max(60, Math.min(85, this.systemStats.storageUsage + (Math.random() -
            0.5) * 2));
        },
