...
@section('page-header')
<div
  class="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
  ...
  <div class="flex items-center space-x-2">
    ...
    <button @click="$store.mobile.toggleView()" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
      ...
      <svg class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"
        x-show="$store.mobile.view === 'cards'">
...
<div x-data="mobileTickets()" x-init="init()" class="relative">
  ...
  <div x-show="pullToRefreshState.visible"
    ...
    x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform -translate-y-full"
  ...
  <div x-show="$store.mobile.showSearch" x-transition:enter="transition ease-out duration-200"
    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
    x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
    x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 bg-white dark:bg-gray-900">
    ...
    <div class="flex items-center p-4 border-b border-gray-200 dark:border-gray-700">
      ...
      <input type="search" placeholder="Search events, teams, venues..."
        ...
        x-model="searchQuery" @input="performSearch()" x-ref="searchInput">
    </div>
  ...
  <div x-show="$store.mobile.showFilters" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
      ...
      <div class="px-4 pb-6">
          ...
          <div class="flex flex-wrap gap-2">
            <template x-for="sport in popularSports" :key="sport.id">
              <button @click="toggleSportFilter(sport.id)"
                :class="selectedSports.includes(sport.id) ? 'bg-blue-500 text-white' :
        ...
        <div class="mb-6">
          ...
          <div class="space-y-3">
            ...
            <input type="range" x-model="priceRange.min" min="0" max="1000" step="25"
              class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer">
            <input type="range" x-model="priceRange.max" min="0" max="1000" step="25"
              class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer">
          </div>
        </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <template x-for="period in datePeriods" :key="period.id">
              <button @click="selectDatePeriod(period.id)"
                :class="selectedDatePeriod === period.id ? 'bg-blue-500 text-white' :
                    'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'"
  ...
  <div class="pb-20" x-ref="ticketContainer" @touchstart="handleTouchStart($event)"
        ...
        <div class="relative bg-white dark:bg-gray-900 p-4">
          <div class="flex items-start justify-between">
            <div class="flex-1 pr-4">
              <div class="flex items-center space-x-2 mb-2">
                <span
                  class="px-2 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 text-xs font-medium rounded-full"
                  x-text="ticket.sport"></span>
                <span
                  class="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-full"
                  x-text="ticket.platform"></span>
              </div>

              <h3 class="font-semibold text-gray-900 dark:text-white text-lg mb-1" x-text="ticket.title"></h3>

              <p class="text-gray-600 dark:text-gray-400 text-sm mb-2" x-text="ticket.venue"></p>
          ...
          <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-100 dark:border-gray-800">
            <div class="flex space-x-2">
              <button @click="toggleFavorite(ticket)" :class="ticket.isFavorite ? 'text-red-500' : 'text-gray-400'"
                class="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                <svg class="w-5 h-5" :fill="ticket.isFavorite ? 'currentColor' : 'none'" stroke="currentColor"
                  viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
              ...
              <button @click="createAlert(ticket)"
                ...
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M15 17h5l-5 5v-5zM4 19h6v-2H4v2zM4 14h16v-2H4v2zM4 9h16V7H4v2z" />
                </svg>
              </button>
  ...
  <div class="fixed bottom-6 right-6 z-30">
    <button @click="scrollToTop()" x-show="showScrollTop"
      x-transition:enter="transition ease-out duration-200 transform" x-transition:enter-start="scale-0 opacity-0"
      x-transition:enter-end="scale-100 opacity-100" x-transition:leave="transition ease-in duration-150 transform"
      x-transition:leave-start="scale-100 opacity-100" x-transition:leave-end="scale-0 opacity-0"
