...
@section('description', 'Register for HD Tickets - Professional sports event ticket monitoring platform. 7-day free trial, subscription-based access.')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-12">
    ...
    <div class="max-w-md mx-auto">
      ...
      <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700 p-8">
          ...
          @foreach($providers as $provider => $config)
            @if($config['enabled'])
              <a href="{{ route('oauth.redirect', ['provider' => $provider]) }}"
                 class="w-full flex justify-center items-center px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
                @if($provider === 'google')
        ...
        <div class="relative mb-6">
          ...
          <div class="relative flex justify-center text-sm">
            <span class="px-3 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">Or register with email</span>
          </div>
        </div>
        @endif

        <!-- Registration Form -->
        <form method="POST" action="{{ route('register.public') }}" x-data="registrationForm()" @submit.prevent="submitForm()" novalidate>
          @csrf
          ...
          <div class="space-y-4">
            ...
            <div x-data="{ showPassword: false }">
              ...
              <div class="relative">
                <input :type="showPassword ? 'text' : 'password'" 
                       ...
                       class="w-full px-4 py-3 pr-12 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white transition-colors">
                <button type="button" 
            ...
            <div x-data="{ showPasswordConfirm: false }">
              ...
              <div x-show="touched.password_confirmation && form.password_confirmation" class="mt-1">
                <p x-show="form.password !== form.password_confirmation" class="text-sm text-red-600">✗ Passwords do not match</p>
                <p x-show="form.password === form.password_confirmation && form.password_confirmation.length > 0" class="text-sm text-green-600">✓ Passwords match</p>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-4">
              <label class="flex items-start space-x-3 cursor-pointer">
                ...
                <span class="text-sm text-gray-700 dark:text-gray-300">
                  I agree to the 
                  <a href="{{ route('legal.terms-of-service') }}" target="_blank" class="text-blue-600 hover:text-blue-500 underline">
                    Terms of Service
                  </a> and 
                  <a href="{{ route('legal.privacy-policy') }}" target="_blank" class="text-blue-600 hover:text-blue-500 underline">
                    Privacy Policy
                  </a>
                </span>
              </label>
          ...
          <!-- Security Options -->
          <div class="mt-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Security Options</h3>
            ...
            <label class="flex items-start space-x-3 cursor-pointer">
              <input type="checkbox" 
                     ...
                     class="mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
              <div class="flex-1">
                <span class="text-sm font-medium text-gray-900 dark:text-white">Enable Two-Factor Authentication (2FA)</span>
            ...
            <div class="flex items-start">
              ...
              <div>
                ...
                <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                  This service is provided "as-is" with no warranty or money-back guarantee. You will receive a 7-day free trial, after which subscription fees apply.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="mt-8">
            <button type="submit" 
                    ...
                    :class="isFormValid() && !submitting ? 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-500' : 'bg-gray-400 cursor-not-allowed'"
      ...
      <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
        <div class="text-gray-700 dark:text-gray-300">
          <div class="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 bg-blue-100 dark:bg-blue-900/30">
            <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
            </svg>
          </div>
          <h3 class="font-semibold text-sm">Real-Time Monitoring</h3>
          <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Track prices across 50+ platforms</p>
        </div>
        <div class="text-gray-700 dark:text-gray-300">
          <div class="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 bg-emerald-100 dark:bg-emerald-900/30">
          ...
          <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Get notified when prices drop</p>
        </div>
        <div class="text-gray-700 dark:text-gray-300">
          <div class="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 bg-purple-100 dark:bg-purple-900/30">
            <svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
            </svg>
          </div>
          <h3 class="font-semibold text-sm">Enterprise Security</h3>
          <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">GDPR compliant with 2FA support</p>
        </div>
      </div>
