...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
  ...
  <head>
    ...
    <meta name="apple-mobile-web-app-title" content="HD Tickets Registration">
    <meta name="mobile-web-app-capable" content="yes">
    <meta name="theme-color" content="#1e40af">
    <meta name="msapplication-navbutton-color" content="#1e40af">
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>Create Your Account - {{ config('app.name', 'HD Tickets') }}</title>
    <meta name="description"
      content="Join HD Tickets - Professional Sports Events Entry Tickets Monitoring System. Create your account and start tracking ticket prices today.">
  ...
  <body class="h-full bg-gradient-to-br from-indigo-50 via-white to-purple-50 font-sans antialiased">
    ...
    <div class="min-h-screen" x-data="comprehensiveRegistration()">
      {{-- Progress Header --}}
      <div class="sticky top-0 z-40 bg-white/90 backdrop-blur-sm border-b border-gray-200">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <span class="text-lg font-semibold text-gray-900">Create Your Account</span>
            ...
            <div class="flex items-center space-x-2 text-sm text-gray-500">
              ...
              <span>of</span>
              <span>{{ count($availableRoles ?? []) > 1 ? '5' : '4' }}</span>
            </div>
          </div>
          ...
          <div class="mt-4 w-full bg-gray-200 rounded-full h-2">
            <div class="bg-indigo-600 h-2 rounded-full transition-all duration-500"
              :style="`width: ${(currentStep / {{ count($availableRoles ?? []) > 1 ? '5' : '4' }}) * 100}%`"></div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-4xl mx-auto px-4 py-8" x-data="comprehensiveRegistration()">
        ...
        <div x-show="currentStep === 1" x-transition:enter="transition ease-out duration-300"
          ...
          <div class="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {{-- Customer Account --}}
            <div class="relative">
              <input type="radio" id="customer" name="account_type" value="customer" x-model="form.accountType"
                class="sr-only">
              <label for="customer" class="block cursor-pointer">
                <div class="bg-white rounded-2xl shadow-lg border-2 transition-all duration-200 p-8 h-full"
                  :class="form.accountType === 'customer' ? 'border-indigo-500 ring-2 ring-indigo-200' :
            ...
            {{-- Agent Account --}}
            <div class="relative">
              <input type="radio" id="agent" name="account_type" value="agent" x-model="form.accountType"
                class="sr-only">
              <label for="agent" class="block cursor-pointer">
                <div class="bg-white rounded-2xl shadow-lg border-2 transition-all duration-200 p-8 h-full"
                  :class="form.accountType === 'agent' ? 'border-purple-500 ring-2 ring-purple-200' :
                  ...
                  <div class="text-center mb-6">
                    <div class="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4">
                      <svg class="w-8 h-8 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2-2v2m8 0V2m0 4v2a2 2 0 01-2 2H10a2 2 0 01-2-2V6m8 0H8" />
                      </svg>
                    </div>
          ...
          <div class="flex justify-end mt-8">
            <button @click="nextStep()" :disabled="!form.accountType"
        ...
        <div x-show="currentStep === 2" x-transition:enter="transition ease-out duration-300"
            ...
            <form @submit.prevent="validatePersonalInfo" class="space-y-6">
              <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" id="first_name" x-model="form.firstName" required
                ...
                <div>
                  ...
                  <input type="text" id="last_name" x-model="form.lastName" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors"
              ...
              <div>
                ...
                <input type="email" id="email" x-model="form.email" @blur="checkEmailAvailability" required
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors"
                ...
                <div x-show="form.password.length > 0" class="mt-2">
                  <div class="flex items-center space-x-2">
                    <div class="flex-1 bg-gray-200 rounded-full h-2">
                      <div class="h-2 rounded-full transition-all duration-300" :class="passwordStrengthColor"
                        :style="`width: ${passwordStrengthPercent}%`"></div>
                    </div>
                    <span class="text-sm font-medium" :class="passwordStrengthColor.replace('bg-', 'text-')"
                      x-text="passwordStrengthText"></span>
                  </div>
                  <div class="mt-2 space-y-1 text-sm">
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" :class="form.password.length >= 8 ? 'text-green-500' : 'text-gray-300'"
                        fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                          d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                          clip-rule="evenodd" />
                      </svg>
                      <span :class="form.password.length >= 8 ? 'text-green-600' : 'text-gray-500'">At least 8
                        characters</span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" :class="/[A-Z]/.test(form.password) ? 'text-green-500' : 'text-gray-300'"
                        fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                          d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                          clip-rule="evenodd" />
                      </svg>
                      <span :class="/[A-Z]/.test(form.password) ? 'text-green-600' : 'text-gray-500'">One uppercase
                        letter</span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" :class="/[0-9]/.test(form.password) ? 'text-green-500' : 'text-gray-300'"
                        fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                          d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                          clip-rule="evenodd" />
              ...
              <div>
                ...
                <input type="password" id="password_confirmation" x-model="form.passwordConfirmation" required
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors"
                  placeholder="Confirm your password">
                <p x-show="form.passwordConfirmation && form.password !== form.passwordConfirmation"
          ...
          <div class="flex justify-between mt-8">
            ...
            <button @click="validatePersonalInfo()" :disabled="!isPersonalInfoValid"
        ...
        <div x-show="currentStep === 3" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-gray-600 mb-4">Our terms outline your rights and responsibilities when using HD
                    Tickets.</p>
                  ...
                  <div class="flex items-center space-x-3">
                    <input type="checkbox" id="accept_terms" x-model="form.agreements.terms"
                      class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                    <label for="accept_terms" class="text-sm text-gray-700">
                      I agree to the
                      <a href="/legal/terms" target="_blank"
                        class="text-indigo-600 hover:text-indigo-500 underline">Terms of Service</a>
                    </label>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center space-x-3">
                    <input type="checkbox" id="accept_privacy" x-model="form.agreements.privacy"
                      class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                    <label for="accept_privacy" class="text-sm text-gray-700">
                      I agree to the
                      <a href="/legal/privacy" target="_blank"
                        class="text-indigo-600 hover:text-indigo-500 underline">Privacy Policy</a>
                    </label>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                  ...
                  <div class="space-y-3">
                    <div class="flex items-center space-x-3">
                      <input type="checkbox" id="consent_processing" x-model="form.agreements.dataProcessing"
                        class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                    ...
                    <div class="flex items-center space-x-3">
                      <input type="checkbox" id="consent_marketing" x-model="form.agreements.marketing"
                        class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
          ...
          <div class="flex justify-between mt-8">
            ...
            <button @click="nextStep()" :disabled="!areAgreementsValid"
        ...
        <div x-show="currentStep === 4" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="mb-8">
              ...
              <div class="space-y-4">
                ...
                <div x-show="form.phone" class="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                  ...
                  <div class="flex items-center space-x-2">
                    <input type="checkbox" id="enable_sms" x-model="form.enableSMSVerification"
          ...
          <div class="flex justify-between mt-8">
            <button @click="previousStep()" :disabled="isSubmitting"
