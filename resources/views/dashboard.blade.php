<x-unified-layout title="Dashboard" subtitle="Your Sports Ticket Monitoring Hub">
  <x-slot name="headerActions">
    <div class="flex items-center space-x-3">
      ...
      <x-ui.button variant="ghost" size="sm" x-data="{}"
        @click="AppCore.getModule('websocket')?.reconnect()" title="Reconnect WebSocket">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="hd-heading-2 !text-white !mb-0"
                x-text="stats.active_monitors || '{{ $stats['active_monitors'] ?? 0 }}'">
                {{ $stats['active_monitors'] ?? 0 }}</p>
              <div class="mt-1">
        ...
        <x-ui.card-content>
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="hd-heading-2 !text-white !mb-0"
                x-text="stats.alerts_today || '{{ $stats['alerts_today'] ?? 0 }}'">{{ $stats['alerts_today'] ?? 0 }}</p>
              <div class="mt-1">
            ...
            <svg class="w-8 h-8 text-white/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M15 17h5l-5 5v-5zM15 17h5l-5 5v-5zM12 17H7a3 3 0 01-3-3V5a3 3 0 013-3h5"></path>
            </svg>
          </div>
        </x-ui.card-content>
      </x-ui.card>
        ...
        <x-ui.card-content>
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="hd-heading-2 !text-white !mb-0"
                x-text="stats.price_drops || '{{ $stats['price_drops'] ?? 0 }}'">{{ $stats['price_drops'] ?? 0 }}</p>
              <div class="mt-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
      ...
      <x-ui.card>
        <x-ui.card-header title="Recent Alerts">
          <x-ui.button href="{{ route('tickets.alerts.index') }}" variant="ghost" size="sm">View
            All</x-ui.button>
        </x-ui.card-header>
        <x-ui.card-content>
          <div class="space-y-3">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            @if (Auth::user()->isAdmin() || Auth::user()->isAgent())
              <a href="{{ route('tickets.scraping.index') }}"
                ...
                <svg class="w-8 h-8 text-blue-600 mb-2 group-hover:scale-110 transition-transform" fill="none"
              ...
              <a href="{{ route('tickets.alerts.index') }}"
                ...
                <svg class="w-8 h-8 text-green-600 mb-2 group-hover:scale-110 transition-transform" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <a href="{{ route('purchase-decisions.index') }}"
                ...
                <svg class="w-8 h-8 text-purple-600 mb-2 group-hover:scale-110 transition-transform" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <a href="{{ route('ticket-sources.index') }}"
                ...
                <svg class="w-8 h-8 text-orange-600 mb-2 group-hover:scale-110 transition-transform" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  @push('scripts')
    <script src="{{ asset('js/dashboard-enhancements.js') }}" defer></script>
    <script>
