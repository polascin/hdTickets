...
class AnalyticsService {
     ...
     */
    async initializeGA4() {
        const GA_MEASUREMENT_ID = window.HD_TICKETS_CONFIG?.ga4MeasurementId || 'G-XXXXXXXXXX';
    ...
    setupConversionFunnels() {
        this.conversionFunnels.set('user_registration', {
            steps: ['welcome_view', 'signup_click', 'form_start', 'form_submit', 'registration_complete'],
            currentStep: 0
        });
        
        this.conversionFunnels.set('ticket_purchase', {
            steps: ['ticket_view', 'ticket_click', 'purchase_intent', 'checkout_start', 'purchase_complete'],
            currentStep: 0
        });
    }

    updateConversionFunnel(eventName, eventData) {
        ...
        try {
            const response = await fetch('/api/v1/analytics/event', {
                ...
                body: JSON.stringify({
                    event: eventName,
                    data: {
                        event_category: this.eventCategories[eventData.category] || 'general',
                        ...eventData,
                        session_id: this.sessionId,
    ...
    setupSportsEventListeners() {
        ...
        document.addEventListener('click', (event) => {
            const target = event.target.closest('[data-sport], [data-team], [data-analytics]');
            if (target) {
