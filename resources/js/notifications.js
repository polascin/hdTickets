...
class NotificationSystem {
    ...
    processNotificationQueue() {
        ...
        const notifications = this.notificationQueue.splice(0, this.config.maxBatchSize);
    ...
    async processNotification(notification) {
        try {
            ...
            if ('vibrate' in navigator && notification.vibrate && this.preferences.vibrateEnabled) {
                navigator.vibrate(notification.vibrate);
            }

            // Show browser notification
            if (this.preferences.browserNotifications && 
                document.visibilityState === 'hidden' || notification.priority === 'high') {
    ...
    handleDisconnection() {
        this.showSystemNotification('Real-time notifications disconnected. Attempting to reconnect...', 'warning');
    ...
    attemptReconnection() {
        if (this.retryCount >= this.config.retryAttempts) {
            this.showSystemNotification('Unable to establish real-time connection. Please refresh the page.', 'error');
            return;
        }
    ...
    handleInitializationError(error) {
        ...
        this.showSystemNotification('Notification system failed to start. Some features may not work correctly.', 'error');
    }
