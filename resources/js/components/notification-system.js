...
document.addEventListener('alpine:init', () => {
  Alpine.data('notificationSystem', () => ({
    ...
    getClasses(type) {
      const baseClasses =
        'transform transition-all duration-300 ease-in-out p-4 rounded-lg shadow-lg border-l-4 max-w-sm';
      const typeClasses = {
        success:
        ...
        warning:
          'bg-yellow-50 border-yellow-400 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200',
  ...
  window.notify = {
    ...
    success: (message, duration) =>
      window.notify.show(message, 'success', duration),
    error: (message, duration) =>
      window.notify.show(message, 'error', duration),
    warning: (message, duration) =>
      window.notify.show(message, 'warning', duration),
    info: (message, duration) => window.notify.show(message, 'info', duration),
    goal: message => {
      const event = new CustomEvent('show-notification', {
        detail: {
          message: `⚽ ${message || 'Goal! You scored with HD Tickets!'}`,
          type: 'success',
        },
      });
      window.dispatchEvent(event);
