...
class ApiAuthenticationMiddleware
{
    ...
    public function handle(Request $request, Closure $next, ...$roles): Response
    {
        ...
        $decayMinutes = $this->getDecayMinutesForUser($user, $request);
     ...
     */
    private function getDecayMinutesForUser($user, Request $request): int
    {
    ...
    private function detectSuspiciousActivity($user, Request $request): bool
    {
        ...
        $activity = array_filter($activity, fn($entry) => $entry['timestamp'] > $now - 3600);
