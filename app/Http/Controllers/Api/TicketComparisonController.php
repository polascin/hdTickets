...
class TicketComparisonController extends Controller
{
    ...
    private function calculateValueScore(Ticket $ticket): float
    {
        ...
        elseif ($price <= 100) $score += 15;
        elseif ($price <= 200) $score += 10;
        elseif ($price > 300) $score -= 10;
        
        // Seller rating factor
        if ($ticket->seller_rating >= 4.5) $score += 15;
        elseif ($ticket->seller_rating >= 4.0) $score += 10;
        elseif ($ticket->seller_rating >= 3.5) $score += 5;
        elseif ($ticket->seller_rating < 3.0) $score -= 10;
        
        // Features factor
        if ($ticket->instant_download) $score += 5;
        if ($ticket->mobile_transfer) $score += 5;
        if ($ticket->has_guarantee) $score += 10;
        if ($ticket->parking_included) $score += 5;
        
        // Platform reliability factor
        $reliablePlatforms = ['Ticketmaster', 'StubHub', 'Viagogo'];
        if (in_array($ticket->platform, $reliablePlatforms)) $score += 10;
        
        // Availability factor
        if (!$ticket->is_available) $score = 0;
    ...
    private function calculatePriceCompetitiveness($platformTickets): string
    {
        ...
        if ($avgPrice < 100) return 'very_competitive';
        if ($avgPrice < 200) return 'competitive';
        if ($avgPrice < 300) return 'moderate';
        return 'premium';
    }
    ...
    private function determineMarketTrend($tickets): string
    {
        ...
        if ($avgMarkup > 50) return 'sellers_market';
        if ($avgMarkup < 20) return 'buyers_market';
        return 'balanced';
    }
    ...
    private function calculateAvailabilityPressure($tickets): string
    {
        ...
        if ($availablePercent > 50) return 'medium';
        return 'high';
    }
    ...
    private function determinePricingStrategy($tickets): string
    {
        ...
        if ($priceVariance > 100) return 'diverse_pricing';
        if ($priceVariance < 20) return 'uniform_pricing';
