...
class SocialProofController extends Controller
{
    ...
    private function calculateEventDemandLevel($tickets): string
    {
        ...
        if ($availabilityRatio < 0.2) $score += 30;
        elseif ($availabilityRatio < 0.5) $score += 20;
        elseif ($availabilityRatio < 0.8) $score += 10;
        
        // Price trends
        $avgPrice = $tickets->avg('current_price');
        if ($avgPrice > 200) $score += 20;
        elseif ($avgPrice > 100) $score += 10;
        
        // Platform diversity
        $platformCount = $tickets->unique('platform')->count();
        if ($platformCount > 5) $score += 15;
        elseif ($platformCount > 3) $score += 10;
        
        // Views activity
        $totalViews = $tickets->sum('views_count');
        if ($totalViews > 1000) $score += 15;
        elseif ($totalViews > 500) $score += 10;
    ...
    private function calculateUrgencyScore($tickets): int
    {
        ...
        if ($firstTicket && $firstTicket->event_date) {
            ...
            if ($daysToEvent <= 1) $score += 20;
            elseif ($daysToEvent <= 7) $score += 15;
            elseif ($daysToEvent <= 30) $score += 10;
        }
    ...
    {
        if ($listings > 100) return 'very_high';
        if ($listings > 50) return 'high';
        if ($listings > 20) return 'medium';
        return 'low';
    }
    ...
    private function getEventDemandIndicator(Event $event): string
    {
        ...
        if ($event->popularity_score > 80) return 'very_high';
        if ($event->popularity_score > 60) return 'high';
        if ($event->popularity_score > 40) return 'medium';
        return 'low';
    }
    ...
    private function determinePriceTrend($tickets): string
    {
        ...
        if ($tickets->isEmpty()) return 'stable';
        
        $avgPrice = $tickets->avg('current_price');
        if ($avgPrice > 200) return 'increasing';
        if ($avgPrice < 50) return 'decreasing';
        return 'stable';
    }
    ...
    {
        $firstTicket = $tickets->first();
        if (!$firstTicket || !$firstTicket->event_date) return 'none';
        
        $daysToEvent = now()->diffInDays($firstTicket->event_date);
        if ($daysToEvent <= 1) return 'critical';
        if ($daysToEvent <= 7) return 'high';
        if ($daysToEvent <= 30) return 'medium';
        return 'low';
    }
    ...
    private function calculateSingleTicketDemand(Ticket $ticket): string
    {
        ...
        if (($ticket->views_count ?? 0) > 50) $score += 30;
        elseif (($ticket->views_count ?? 0) > 20) $score += 20;
        
        if ($ticket->current_price < 100) $score += 20;
        if ($ticket->quantity <= 2) $score += 25;
        if ($ticket->seller_rating >= 4.0) $score += 15;
