...
class UserActivityController extends Controller
{
    ...
    {
        $purchases = PurchaseAttempt::whereHas('purchaseQueue', function($query) use ($userId) {
    ...
    private function getWatchlistData(int $userId, Carbon $startDate): array
    {
        ...
        $watchlistTickets = ScrapedTicket::where('scraped_at', '>=', $startDate)
            ->where(function($query) use ($favoriteTeams, $favoriteVenues) {
    ...
    private function getChartData(int $userId, Carbon $startDate): array
    {
        ...
        $purchaseAttempts = PurchaseAttempt::whereHas('purchaseQueue', function($query) use ($userId) {
                $query->where('user_id', $userId);
            })
