...
{
    public function __construct(protected TicketApiManager $apiManager)
    {
    }
    ...
    public function index(): View
    {
        ...
        return view('ticket-api.index', ['availablePlatforms' => $availablePlatforms]);
    }
     ...
     * Get  event
     */
    public function getEvent(Request $request, string $platform, string $eventId): RedirectResponse
    {
        try {
     ...
     * ImportEvents
     */
    public function importEvents(Request $request): RedirectResponse
    {
        $request->validate([
            'query'       => 'required|string|max:255',
    ...
    public function suggestions(Request $request)
    {
        ...
        $cacheKey = 'suggestions_' . md5((string) $query);
        ...
        try {
            $suggestions = Cache::remember($cacheKey, 1800, function () use ($query) { // 30 minutes cache
                ...
                usort($suggestions, function ($a, $b) use ($query): int|float {
                    $aExact = stripos((string) $a->text, (string) $query) === 0 ? 1 : 0;
                    $bExact = stripos((string) $b->text, (string) $query) === 0 ? 1 : 0;
