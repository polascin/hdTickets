...
class DashboardController extends Controller
{
    ...
    public function getUserMetrics(Request $request)
    {
        ...
        return Cache::remember($cacheKey, 600, function () use ($user, $request) {
            try {
                ...
                if (!empty($favoriteTeams)) {
                    $recommendationsQuery->where(function ($q) use ($favoriteTeams) {
     ...
     */
    private function warmFrequentlyAccessedData($tickets)
    {
        try {
    ...
    private function getAvailabilityTrend($event)
    {
        ...
        if ($event->ticket_count > 20 && $event->high_demand_count < 5) {
            return 'increasing';
        } elseif ($event->ticket_count < 10 && $event->high_demand_count > 8) {
            return 'decreasing';
        }
        return 'stable';
    }
    ...
    private function getUserFavoriteSports($userId)
    {
        try {
            return TicketAlert::forUser($userId)
                ->active()
                ->join('scraped_tickets', function($join) {
    ...
    private function calculateOverallSystemHealth($platformStats)
    {
        ...
        if ($healthPercentage >= 80) return ['status' => 'healthy', 'score' => round($healthPercentage)];
        if ($healthPercentage >= 60) return ['status' => 'degraded', 'score' => round($healthPercentage)];
