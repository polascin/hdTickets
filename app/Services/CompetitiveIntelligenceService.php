...
class CompetitiveIntelligenceService
{
    ...
    private function determineQuadrant($price, $volume): string
    {
        ...
        if ($highPrice && $highVolume) return 'premium_leader';
        if ($highPrice && !$highVolume) return 'niche_premium';
        if (!$highPrice && $highVolume) return 'volume_leader';
    ...
    private function calculateMarketConcentration(Collection $marketShare): array
    {
        ...
        elseif ($hhi > 1500) $concentration = 'moderate';
    ...
    private function calculateCompetitiveIntensity(Collection $marketShare): string
    {
        ...
        if ($topThreeShare > 75) return 'low';
        if ($topThreeShare > 50) return 'moderate';
        return 'high';
    }
    ...
    private function identifyPricingStrategy(Collection $tickets): string
    {
        ...
        if ($cv < 0.1) return 'fixed_pricing';
        if ($cv < 0.3) return 'tiered_pricing';
