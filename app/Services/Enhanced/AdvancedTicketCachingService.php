...
class AdvancedTicketCachingService
{
    ...
    const TTL_ULTRA_SHORT = 30;      // 30 seconds for real-time data
    const TTL_SHORT = 120;           // 2 minutes for frequent updates
    const TTL_MEDIUM = 300;          // 5 minutes for moderate updates
    const TTL_LONG = 900;            // 15 minutes for less frequent data
    const TTL_EXTENDED = 3600;       // 1 hour for stable data
    const TTL_DAILY = 86400;         // 24 hours for daily statistics
    
    // Cache key prefixes
    const PREFIX_TICKETS = 'tickets:';
