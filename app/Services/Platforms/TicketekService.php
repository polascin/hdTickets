...
class TicketekService extends BasePlatformService
{
    ...
    private function parseEventDetailsHtml(string $html, string $eventUrl, string $region): array
    {
        ...
        foreach ($dateNodes as $node) {
            ...
            if ($perfDate) {
                if (!$eventDate) $eventDate = $perfDate;
    ...
    private function categorizeEvent(string $title, array $eventData = []): string
    {
        ...
        if (isset($eventData['type'])) {
            ...
            if (str_contains($type, 'sport')) return 'sports';
            if (str_contains($type, 'music') || str_contains($type, 'concert')) return 'music';
            if (str_contains($type, 'theatre') || str_contains($type, 'play')) return 'theatre';
        }

        // Check genre
        if (isset($eventData['genre'])) {
            $genre = strtolower($eventData['genre']);
            if (str_contains($genre, 'rock') || str_contains($genre, 'pop') || str_contains($genre, 'jazz')) return 'music';
            if (str_contains($genre, 'comedy')) return 'comedy';
        }
