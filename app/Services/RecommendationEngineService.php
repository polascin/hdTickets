...
class RecommendationEngineService
{
    ...
    public function generateRecommendations(User $user, array $options = []): array
    {
        ...
        return Cache::remember($cacheKey, $this->config['cache_ttl'], function () use ($user, $options) {
            ...
            try {
                ...
                $recommendations = [
                    ...
                    'venues' => $this->recommendVenues($user, $userContext),
                    'meta' => [
    ...
    private function buildUserContext(User $user): array
    {
        ...
        return [
            ...
            'price_sensitivity' => $this->calculatePriceSensitivity($user, $purchaseHistory),
            'favorite_sports' => $this->extractFavoriteSports($purchaseHistory, $behaviorData),
    ...
    private function recommendEvents(User $user, array $userContext, array $options = []): array
    {
        ...
        $strategies = [
            ...
            'trending_events' => $this->getTrendingEvents($query, $userContext),
    ...
    private function recommendPricingStrategies(User $user, array $userContext): array
    {
        ...
        $strategies[] = [
            ...
            'recommended_thresholds' => $this->calculateOptimalPriceThresholds($user, $userContext),
            'confidence' => 0.82
        ];
    ...
    private function recommendTeams(User $user, array $userContext): array
    {
        ...
        $allTeams = collect()
            ->merge($purchasedTeams->map(fn($team) => array_merge($team, ['source' => 'purchase_history', 'score' => 0.9])))
            ->merge($collaborativeTeams->map(fn($team) => array_merge($team, ['source' => 'similar_users', 'score' => 0.7])))
            ->merge($trendingTeams->map(fn($team) => array_merge($team, ['source' => 'trending', 'score' => 0.6])));
     ...
     */
    private function recommendVenues(User $user, array $userContext): array
    {
        ...
        $allVenues = collect()
            ->merge($visitedVenues->map(fn($venue) => array_merge($venue, ['source' => 'visited', 'score' => 0.8])))
            ->merge($nearbyVenues->map(fn($venue) => array_merge($venue, ['source' => 'nearby', 'score' => 0.6])))
            ->merge($relevantVenues->map(fn($venue) => array_merge($venue, ['source' => 'interests', 'score' => 0.7])));
    ...
    private function findSimilarUsers(User $user, array $userContext): Collection
    {
        ...
        return Cache::remember($cacheKey, 1800, function () use ($user, $userContext) {
            ...
            $candidates = User::whereHas('ticketPurchases', function ($query) {
    ...
    private function buildUserVector(array $userContext): array
    {
        ...
        foreach ($allSports as $sport) {
            $vector[] = in_array($sport, $userContext['favorite_sports'] ?? []) ? 1 : 0;
        }
        ...
        $vector[] = !empty($userContext['location_preferences']['primary_city']) ? 1 : 0;
     ...
     */
    private function calculatePriceSensitivity(User $user, array $purchaseHistory): float
    {
     ...
     */
    private function extractFavoriteSports(array $purchaseHistory, array $behaviorData): array
    {
        $sportsCount = [];
        ...
        switch ($variant) {
            case 'personalized_heavy':
                ...
                $recommendations['events']['recommendations'] = $this->boostPersonalizedResults(
                    $recommendations['events']['recommendations'], 0.2
                );
                break;
            ...
            case 'collaborative_heavy':
                ...
                $recommendations['events']['recommendations'] = $this->boostCollaborativeResults(
                    $recommendations['events']['recommendations'], 0.15
                );
                break;
            ...
            case 'trending_focus':
                ...
                $recommendations['events']['recommendations'] = $this->boostTrendingResults(
                    $recommendations['events']['recommendations'], 0.1
                );
                break;
        }
     ...
     */
    private function getFallbackRecommendations(User $user): array
    {
        return [
            'events' => [
     ...
     */
    private function calculateOptimalPriceThresholds(User $user, array $userContext): array
    {
        $priceSensitivity = $userContext['price_sensitivity'];
        $purchaseHistory = $userContext['purchase_history'];
        ...
        $avgPrice = collect($purchaseHistory)->pluck('price')->avg();
     ...
     */
    private function getEventsBaseQuery(array $filters): \Illuminate\Database\Query\Builder
    {
     ...
     */
    private function getPreferenceBasedEvents($query, array $userContext): Collection
    {
        ...
        return $preferenceQuery->limit(15)
            ->get()
            ->map(function ($event) {
                return array_merge((array)$event, ['score' => 0.85]);
            });
    }
     ...
     */
    private function getTrendingEvents($query, array $userContext): Collection
    {
        ...
        return $trendingQuery->leftJoin('ticket_purchases', 'scraped_tickets.id', '=', 'ticket_purchases.ticket_id')
            ...
            ->get()
            ->map(function ($event) {
                return array_merge((array)$event, ['score' => 0.65]);
            });
    }
    ...
    private function deduplicateAndScore(Collection $events, array $userContext): Collection
    {
        return $events->groupBy('id')
            ->map(function ($duplicates) use ($userContext) {
            ...
            ->filter(function ($event) {
                return $event['final_score'] >= $this->config['min_confidence_score'];
            })
    ...
    {
        return $recommendations->filter(function ($event) use ($user) {
            ...
            if (isset($event['availability']) && $event['availability'] === 'sold_out') {
                return false;
            }
