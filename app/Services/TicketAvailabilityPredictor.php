...
class TicketAvailabilityPredictor
{
    ...
    {
        if (count($priceHistory) < 2) return 0;
    ...
    protected function calculatePriceTrend(array $priceHistory, int $days): float
    {
        ...
        if (count($recentPrices) < 2) return 0;
    ...
    protected function categorizeEventType(string $eventName): int
    {
        ...
        if (strpos($eventName, 'concert') !== false) return 1;
        if (strpos($eventName, 'sports') !== false) return 2;
        if (strpos($eventName, 'theater') !== false) return 3;
        if (strpos($eventName, 'comedy') !== false) return 4;
        return 0; // Other
    }
