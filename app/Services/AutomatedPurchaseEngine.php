...
class AutomatedPurchaseEngine
{
    ...
    private function calculateUserPreferenceScore(ScrapedTicket $ticket, User $user): float
    {
        ...
        if (isset($preferences['preferred_sections']) && 
            in_array($ticket->section, $preferences['preferred_sections'])) {
            $score += 20;
        }

        // Preferred platforms
        if (isset($preferences['preferred_platforms']) && 
            in_array($ticket->platform, $preferences['preferred_platforms'])) {
            $score += 15;
        }
    ...
    private function getPlatformSuccessRate(string $platform): float
    {
        ...
        return Cache::remember($cacheKey, now()->addHours(1), function () use ($platform) {
            ...
            if ($total === 0) return 70; // Default rate
    ...
    {
        if ($price < 100) return 85;
        if ($price < 300) return 80;
    ...
    private function categorizeEvent($eventTitle): string
    {
        ...
        if (preg_match('/concert|music|band|singer/', $title)) return 'concert';
        if (preg_match('/game|match|sports|football|basketball/', $title)) return 'sports';
        if (preg_match('/theater|play|musical|opera/', $title)) return 'theater';
        if (preg_match('/festival|fest|fair/', $title)) return 'festival';
