    ...
    public function getPriceVolatilityHeatmapData(array $filters = []): array
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function() use ($filters) {
            ...
            $backgroundColors = $volatilityData->map(function($item) {
                ...
                if ($volatility > 50) return '#ef4444'; // High volatility - red
                if ($volatility > 20) return '#f59e0b'; // Medium volatility - amber
    ...
    public function getSportsCategoryRadarData(array $filters = []): array
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function() use ($filters) {
            ...
            $categoryMetrics = Category::withCount([
                'scrapedTickets as total_tickets' => function($query) use ($startDate, $endDate) {
                    $query->whereBetween('created_at', [$startDate, $endDate]);
                },
                'scrapedTickets as available_tickets' => function($query) use ($startDate, $endDate) {
                    $query->whereBetween('created_at', [$startDate, $endDate])
                          ->where('status', 'available');
                },
                'scrapedTickets as high_demand_tickets' => function($query) use ($startDate, $endDate) {
                    $query->whereBetween('created_at', [$startDate, $endDate])
