...
class JuventusPlugin extends BaseScraperPlugin
{
    /**
    ...
    {
        $this->pluginName = 'Juventus FC';
        $this->platform = 'juventus';
        $this->description = 'Official Juventus FC tickets - Serie A, Champions League, Coppa Italia';
        $this->baseUrl = 'https://www.juventus.com';
        $this->venue = 'Allianz Stadium';
        $this->currency = 'EUR';
        $this->language = 'it-IT';
    ...
    protected function parseSearchResults(string $html): array
    {
        ...
        try {
            // Parse Juventus specific event structure
            $crawler->filter('.match-card, .partita, .match, .evento, .biglietto-item, .fixture')->each(function (Crawler $node) use (&$events) {
                try {
                    ...
                    \Log::warning("Failed to parse Juventus event node", [
                        'error' => $e->getMessage(),
            ...
            \Log::error("Failed to parse Juventus events", [
                'error' => $e->getMessage()
            ]);
        }
