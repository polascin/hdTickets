...
class CelticFCPlugin implements ScraperPluginInterface
{
    ...
    private $baseUrl = 'https://www.celticfc.com';
    ...
    public function getInfo(): array
    {
        return [
            'name'               => 'Celtic FC',
            'description'        => 'Official Celtic FC tickets - Scottish Premiership, European competitions',
            'version'            => '1.0.0',
            'platform'           => 'celtic',
    ...
    {
        $this->enabled = TRUE;
        Log::info('Celtic FC plugin enabled');
    }

    public function disable(): void
    {
        $this->enabled = FALSE;
        Log::info('Celtic FC plugin disabled');
    }

    public function configure(array $config): void
    {
        $this->config = array_merge($this->config, $config);
        Log::info('Celtic FC plugin configured', ['config' => $config]);
    }

    public function scrape(array $criteria): array
    {
        if (! $this->enabled) {
            throw new Exception('Celtic FC plugin is disabled');
        }

        Log::info('Starting Celtic FC scraping', $criteria);
        ...
        try {
            ...
            Log::info('Celtic FC scraping completed', [
                'url'           => $searchUrl,
            ...
            Log::error('Celtic FC scraping failed', [
                'criteria' => $criteria,
                'error'    => $e->getMessage(),
            ]);
    ...
    public function test(): array
    {
        try {
            ...
            return [
                'status'       => 'success',
                'message'      => 'Celtic FC plugin test successful',
            ...
            return [
                'status'  => 'error',
                'message' => 'Celtic FC plugin test failed: ' . $e->getMessage(),
            ];
        }
    }
        ...
        foreach ($eventNodes as $eventNode) {
            try {
                $event = [
                    ...
                    'description'         => '🍀 Celtic FC Official - Celtic Park experience',
                    'last_checked'        => now(),
                ...
                Log::warning('Failed to parse Celtic FC event', ['error' => $e->getMessage()]);
    ...
    private function extractAndParseDate(DOMXPath $xpath, string $selector, DOMElement $context): ?string
    {
        ...
        try {
            ...
            Log::warning('Failed to parse Celtic FC date', ['date_text' => $dateText]);
    ...
    {
        $lastRequest = Cache::get('celtic_last_request', 0);
        ...
        Cache::put('celtic_last_request', microtime(TRUE), 60);
    }
    ...
    private function makeRequest(string $url): string
    {
        try {
            ...
            Log::error('Celtic FC HTTP request failed', [
                'url'   => $url,
                'error' => $e->getMessage(),
            ]);

            throw new Exception('Failed to fetch Celtic FC page: ' . $e->getMessage());
        }
    }
}
