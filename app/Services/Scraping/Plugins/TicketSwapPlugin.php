...
class TicketSwapPlugin extends BaseScraperPlugin
{
    ...
    protected function scrapeTickets(string $searchUrl): array
    {
        try {
            ...
            $crawler->filter('.event-card, .listing-item, .search-result, .ticket-listing')->each(function (Crawler $node) use (&$tickets) {
                try {
    ...
    public function supportsVenue(string $venue): bool
    {
        ...
        return in_array(strtolower($venue), $majorVenues);
    }
