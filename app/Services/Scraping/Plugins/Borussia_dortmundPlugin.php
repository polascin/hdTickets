...
class Borussia_dortmundPlugin extends BaseScraperPlugin
{
    /**
    ...
    protected function initializePlugin(): void
    {
        ...
        $this->platform = 'borussia_dortmund';
        $this->description = 'Official Borussia Dortmund tickets - Bundesliga, Champions League, DFB-Pokal';
        $this->baseUrl = 'https://www.bvb.de';
        $this->venue = 'Signal Iduna Park';
        $this->currency = 'EUR';
        $this->language = 'de-DE';
    ...
    protected function getCapabilities(): array
    {
        return [
            ...
            'season_tickets',
            'stadium_tours',
            'der_klassiker', // vs Bayern Munich
        ];
    }
    ...
    protected function parseSearchResults(string $html): array
    {
        ...
        try {
            // Parse Borussia Dortmund specific event structure
            $crawler->filter('.match-card, .spiel, .match, .event, .ticket-item, .fixture')->each(function (Crawler $node) use (&$events): void {
                try {
                    ...
                    Log::warning('Failed to parse Borussia Dortmund event node', [
                        'error'        => $e->getMessage(),
            ...
            Log::error('Failed to parse Borussia Dortmund events', [
                'error' => $e->getMessage(),
            ]);
        }
