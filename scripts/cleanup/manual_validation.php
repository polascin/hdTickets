...
foreach ($cssFiles as $cssFile) {
    ...
    if (strpos($cssContent, 'mobile') !== FALSE || strpos($cssContent, 'touch-target') !== FALSE) {
        $hasMobileClasses = TRUE;
    }
    if (strpos($cssContent, 'vh') !== FALSE || strpos($cssContent, 'vw') !== FALSE) {
...
foreach (array_slice($viewFiles, 0, 10) as $viewFile) { // Check first 10 files
    ...
    if (strpos($viewContent, 'aria-') !== FALSE) {
        $hasAriaLabels = TRUE;
    }
    if (strpos($viewContent, '<main') !== FALSE || strpos($viewContent, '<nav') !== FALSE ||
        strpos($viewContent, '<header') !== FALSE) {
        $hasSemanticHtml = TRUE;
    }
    if (strpos($viewContent, '<label') !== FALSE && strpos($viewContent, 'for=') !== FALSE) {
